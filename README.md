...
- **Provider abstraction:** Codex and Antigravity supply native CLI agent loops, while OpenRouter models (including GLM and DeepSeek Flash Latest) run as Maverick Agents through the Core-owned hosted tool loop. Google agent models are exposed through Antigravity. Chat and Settings offer no separate text-only model family; speech transcription and synthesis remain independent provider surfaces.
