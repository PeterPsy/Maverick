...
- Agent types persist `skill_activation_mode`. The legacy/default `implicit` mode exposes all enabled workspace skills when `skill_ids` is empty and narrows that set when it is non-empty. `explicit` keeps the catalog out of the baseline prompt and treats non-empty `skill_ids` as the allowlist for turn-local `$skill-id` invocations. Hidden inter-agent messages and orchestration tasks carry only their exact `invoked_skill_ids`; the assigned set remains an allowlist and is never expanded automatically, including when it is empty.
