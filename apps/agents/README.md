...
- Agent types persist `skill_activation_mode`. The legacy/default `implicit` mode exposes all enabled workspace skills when `skill_ids` is empty and narrows that set when it is non-empty. `explicit` keeps the catalog out of the baseline prompt and treats non-empty `skill_ids` as the allowlist for turn-local `$skill-id` invocations. Hidden inter-agent participant turns invoke the immutable assigned set as validated structured ids; an empty assignment retains its documented “all enabled catalog skills” meaning and is resolved server-side at dispatch.
