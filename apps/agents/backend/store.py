...
def normalize_agent_type(payload: dict) -> dict:
    ...
    policy = str(payload.get("execution_mode_policy") or "fixed")
    mode = str(payload.get("default_execution_mode") or "sandbox")
    trace = str(payload.get("trace_verbosity") or "compact")
    if policy not in EXECUTION_MODE_POLICIES:
        raise AgentsValidationError(f"Invalid execution_mode_policy: {policy}")
    if mode not in EXECUTION_MODES:
        raise AgentsValidationError(f"Invalid default_execution_mode: {mode}")
    if trace not in TRACE_VERBOSITIES:
        raise AgentsValidationError(f"Invalid trace_verbosity: {trace}")
    skills = payload.get("codex_skill_ids") if isinstance(payload.get("codex_skill_ids"), list) else []
    return {
        "id": agent_type_id,
        "name": " ".join(str(payload.get("name") or agent_type_id).split()).strip(),
        "description": " ".join(str(payload.get("description") or "").split()).strip(),
        "role_id": role_id,
        "codex_skill_ids": [str(skill_id).strip() for skill_id in skills if str(skill_id).strip()],
