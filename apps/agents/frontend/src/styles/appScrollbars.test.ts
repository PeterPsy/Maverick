...
describe('agents app scrollbars', () => {
  ...
  it('uses a reveal-on-interaction scrollbar for the skill panel', () => {
    const panelStyles = readStyle('panel-scrollbars.css');

    expect(panelStyles).toMatch(/\.agent-skill-picker\s*\{[^}]*scrollbar-color: transparent transparent;/);
    expect(panelStyles).toMatch(/\.agent-skill-picker:focus\s*\{[^}]*scrollbar-color: rgba\(255, 255, 255, 0\.42\) transparent;/);
