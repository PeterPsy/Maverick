...
- App-owned `pinned_apps` state is an ordered list. Pin mutations require workspace registry context, accept only launchable workspace frontends for new pins, and still allow stale entries, including uninstalled or non-launchable apps, to be removed. `pinned_apps.set` preserves the normalized order it receives after filtering, and the Base Shell uses that order for the desktop app rail while keeping App Store itself outside the movable pinned list.
