...
- Mounted workspace app iframes can request temporary full-workspace focus with `{ type: "maverick.shell.workspace-focus", app_id, active }`. The shell only honors the request from a mounted active app frame, hides the desktop sidebar, and expands the workspace frame until the app sends `active: false` or the active app changes.
