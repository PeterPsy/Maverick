...
- M5 structured read models remain parent-mediated and default-off. Every app and widget iframe registration binds exact source/origin to its real owner, workspace, and opaque authenticated shell-session generation. AppShell owns workspace mutations and synchronously withdraws the broker/frame scope and disposes the Storage file broker before calling Core; it publishes a replacement session only after lifecycle transition and registry load complete. Shell API, PWA config, structured broker, file broker, and isolated-launch `401/403` responses share one immediate revocation channel. Every observed authorization failure repeats the synchronous shell signal even while cleanup is pending; only the durable cleanup promise is coalesced, and it cannot cause a received authorization response to be reclassified as a timeout. Late old-scope requests are unavailable before cache lookup; app/widget fan-out requires the sender's registered owner to match `owner_app_id`, while exact top-level shell messages may intentionally cross owners. See `docs/runbooks/pwa_data_cache_m5.md`.
