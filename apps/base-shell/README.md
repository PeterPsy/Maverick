...
- M5 structured read models remain parent-mediated and default-off. Every app and widget iframe registration binds exact source/origin to its real owner, workspace, and opaque authenticated shell-session generation. AppShell synchronously withdraws the broker/frame scope before a session reload or logout, serializes lifecycle mutations, and publishes a replacement session only after transition and registry load complete. Late old-scope requests are unavailable before cache lookup; app/widget fan-out requires the sender's registered owner to match `owner_app_id`, while exact top-level shell messages may intentionally cross owners. See `docs/runbooks/pwa_data_cache_m5.md`.
