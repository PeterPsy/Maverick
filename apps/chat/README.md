...
- `Group chat` is the first advanced product-facing inter-agent mode, but it is not default-on. The Chat composer shows it only when `VITE_MAVERICK_FEATURE_GROUP_CHAT=1`, and the core HTTP API accepts `mode="group_chat"` only when `MAVERICK_FEATURE_GROUP_CHAT=1`. Chat still uses Maverick-owned `InterAgentRun`, participants, events, budget, visibility, replay, cancel, and Agent nodes surfaces; MAF remains adapter/evaluation reference material rather than product runtime ownership.
