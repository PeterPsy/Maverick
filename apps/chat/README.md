...
- Chat declares optional `agent-catalog` and `agent-prompt-materializer` dependencies on the `agent.catalog` and `agent.prompt-materializer` interfaces. The shared composer uses a provider only when the same app satisfies both dependencies, then shows the agent selector for draft chats and materializes the selected agent prompt through that provider before creating the first runtime session. Existing runtime sessions keep their original agent metadata and cannot switch runner from the composer.
...
- Agent response footers expose a frontend-only speaker control beside the copy action when the browser Web Speech API is available. Speech playback uses the browser `speechSynthesis` surface, reads the same collapsed Markdown content visible in the transcript after lightweight Markdown-to-text normalization, does not call the Chat backend, and keeps only one agent response reading at a time.
