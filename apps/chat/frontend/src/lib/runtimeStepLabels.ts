...
export function isNoisyRuntimeLabel(label: string): boolean {
  ...
  return (
    isNonChatFacingProviderEvent(label) ||
...
export function isNonChatFacingProviderEvent(value: unknown): boolean {
...
export function runtimeStepLabel(event: RuntimeEvent): string | null {
  ...
  if (
    !label ||
    providerJsonLabelType(label) ||
    isNoisyRuntimeLabel(label) ||
    isNonChatFacingProviderEvent(event.payload.provider_event_type)
  ) {
    return null;
  }
  return label;
