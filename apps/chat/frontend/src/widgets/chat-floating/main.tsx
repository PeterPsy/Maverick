...
function ChatFloatingMount() {
  ...
  useEffect(() => {
    ...
    setWindows((current) => reconcileWindowsWithThreads(current, threads, "", "", pendingNewChatPreviousThreadByWindow.current));
  }, [threads]);
  ...
  useEffect(() => {
    ...
    function handleWidgetMessage(event: MessageEvent) {
      ...
      if (payload.type === "maverick.app.visibility-changed" && payload.owner_app_id === "chat" && payload.widget_id === "chat-floating") {
        return;
      }
      ...
      debugThreadSync("active-thread-changed", {
        activeThreadId,
        navigationScope,
      ...
      setWindows((current) =>
        reconcileWindowsWithThreads(current, threadsRef.current, activeThreadId, navigationScope, pendingNewChatPreviousThreadByWindow.current),
      );
    }
...
function reconcileWindowsWithThreads(
  windows: ChatWindow[],
  threads: ChatThread[],
  preferredThreadId = "",
  navigationScope = "",
  pendingPreviousThreadByWindow: Map<string, string> = new Map(),
) {
  const firstThreadId = threads[0]?.thread_id || "";
  return windows.map((windowItem) => {
    const pendingPreviousThreadId = pendingPreviousThreadByWindow.get(windowItem.id) || "";
    if (!navigationScope && pendingPreviousThreadId && (!windowItem.threadId || windowItem.threadId === pendingPreviousThreadId)) {
      return windowItem;
    }
    if (!navigationScope) {
