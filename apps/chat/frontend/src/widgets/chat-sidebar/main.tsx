...
function ChatSidebarWidget() {
  ...
  useEffect(() => {
    function handleShellMessage(event: MessageEvent) {
      ...
      const payload = event.data as { active_thread_id?: string; owner_app_id?: string; type?: string };
  ...
  async function removeProject(projectId: string) {
    await Promise.all(threads.filter((thread) => thread.project_id === projectId).map((thread) => updateThread({ thread_id: thread.thread_id, project_id: null })));
