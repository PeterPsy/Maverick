...
- create a chat-renderable checklist with MCP tool `maverick_tasklist`, action `create`, and `payload.sections[].tasks[]`
...
The app's v3 chat widget content kind is `checklist.design`; `chat_render.legacy_kind` preserves the Maverick 2 `design_checklist` identifier. Successful create/read/update responses include `chat_render`; use that structured payload instead of writing checklist JSON in prose.
