...
- Docling is loaded lazily so existing generation and extraction actions continue to run without the optional conversion dependency. Install it with `python3 -m pip install -e '.[document-generator]'` before using `convert_to_markdown`.
