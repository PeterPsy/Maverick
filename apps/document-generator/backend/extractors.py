...
def extract_docx_text(path: Path) -> str:
    with zipfile.ZipFile(path) as archive:
        ...
        chunks = [_xml_text(archive.read(name)) for name in names if name in archive.namelist()]
...
def extract_pptx_text(path: Path) -> str:
    with zipfile.ZipFile(path) as archive:
        ...
        chunks = [_xml_text(archive.read(name)) for name in slide_names]
    return "\n\n".join(chunk for chunk in chunks if chunk)
...
def extract_xlsx_text(path: Path) -> str:
    with zipfile.ZipFile(path) as archive:
        ...
        sheets = [_xlsx_sheet_text(archive.read(name), shared_strings) for name in sheet_names]
