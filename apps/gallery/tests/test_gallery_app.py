...
class GalleryAppTestCase(unittest.TestCase):
    ...
    def test_contract_declares_gallery_surfaces(self) -> None:
        ...
        self.assertIn("gallery_set_view_filter", parsed.contract.capabilities.mcp_tools)
        self.assertIn("gallery_reference_manifest", parsed.contract.capabilities.mcp_tools)
        self.assertIn("gallery_write_file", parsed.contract.capabilities.mcp_tools)
    ...
    def test_backend_full_text_preview_is_not_truncated_by_default(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            generated_root = root / "storage" / "generated"
            generated_root.mkdir(parents=True)
            ...
            self.assertIn("FINAL-MARKER", full_preview["json"]["preview_text"])
            self.assertFalse(full_preview["json"]["preview_text"].endswith("…"))
    ...
    def test_backend_full_table_preview_is_not_truncated_by_default(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            generated_root = root / "storage" / "generated"
            generated_root.mkdir(parents=True)
            ...
            self.assertEqual(len(full_sheet["rows"]), 241)
            self.assertEqual(len(full_sheet["rows"][0]), 55)
            self.assertEqual(full_sheet["rows"][-1][-1], "r239c54")
            self.assertFalse(full_sheet["truncated_rows"])
            self.assertFalse(full_sheet["truncated_columns"])
    ...
    @full_test("full gallery PDF preview suite; run with scripts/test_suite.py --level full")
    ...
    @integration_test("gallery platform integration suite; run with scripts/test_suite.py --level integration")
    def test_bootstrap_installs_gallery_and_exposes_surfaces(self) -> None:
        ...
        self.assertIn("app.gallery.gallery_write_file", [tool.tool_name for tool in tools])
    ...
    def test_platform_backend_and_frontend_mount_gallery(self) -> None:
        ...
        frontend_status, frontend_payload, frontend_headers = self.invoke(app, path="/apps/gallery/", cookie=cookie)
    ...
    @integration_test("gallery platform integration suite; run with scripts/test_suite.py --level integration")
    def test_mcp_and_cli_can_write_gallery_file_content(self) -> None:
        repo_root = self.make_repo_root()
        state = bootstrap_platform_state(start_path=repo_root)
        ...
        mcp_payload = call_mcp_tool(
            tool_name="app.gallery.gallery_write_file",
            context=McpInvocationContext(
                caller_kind="sandbox_agent",
                workspace_id="default",
                agent_id="tester",
