...
Outgoing draft APIs accept `body_text`, optional `body_html`, and optional `reply_to` recipients. Gmail sends HTML drafts as multipart `text/plain` plus `text/html` messages, so provider apps such as CRM can preserve branded transactional email while still supplying a plain-text fallback. Real sending still requires `confirm=true` and the Gmail connection secret selectors declared in the CLI/MCP descriptors.
