...
Gmail sync follows Gmail `nextPageToken` pagination up to the requested `max_threads` limit. `threads.sync` defaults to a 25-thread interactive batch so mounted backend, CLI, and widget calls return before the platform entrypoint timeout; callers that request larger batches should use `continue_cursor=true` and expect them to take proportionally longer. Gmail-backed `threads.list` performs a lightweight mailbox-aware cache prefetch when the connection refresh secret is delivered: `inbox`, `sent`, `drafts`, `starred`, and `trash` map to Gmail search queries such as `in:sent`, and the prefetch covers at least `offset + limit` up to the bounded provider limit. `threads.list` may still return a smaller page for UI readability, its lightweight cache refresh does not replace the persisted import cursor, and when Gmail returns `resultSizeEstimate` the response includes `provider_total_estimate` plus `cache_total_count` so UI pagination can reflect provider scope without hiding cache state.
...
Outgoing draft APIs accept `body_text`, optional `body_html`, optional `reply_to` recipients, and optional `workspace_attachments` entries pointing at `storage/generated/...` or `storage/uploaded/...` files. Mail stores attachment metadata on the draft, then dry-run send previews rebuild the MIME message and return a `confirmation_preview` with current recipients, body, thread id, Storage attachment snapshots including path, size, and sha256, plus an opaque `confirmation_token` and `confirmation_expires_at`. Preview tokens are stored server-side as hashes with a 15 minute TTL and are consumed on confirmed send. Every `confirm=true` send must include a valid preview token so changed recipients, body, thread, sender, or attachment bytes are rejected before external delivery. Gmail and SMTP sends build MIME multipart messages with the selected Storage files attached, and confirmed sends require the Gmail/IMAP connection secret selectors declared in the CLI/MCP descriptors.
