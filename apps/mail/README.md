...
Outgoing draft APIs accept `body_text`, optional `body_html`, optional `reply_to` recipients, and optional `workspace_attachments` entries pointing at `storage/generated/...` or `storage/uploaded/...` files. Mail stores attachment metadata on the draft, returns it in the dry-run confirmation preview, and reads the bytes from Storage only during confirmed send. Gmail and SMTP sends build MIME multipart messages with the selected Storage files attached. Real sending still requires `confirm=true` and the Gmail/IMAP connection secret selectors declared in the CLI/MCP descriptors.
