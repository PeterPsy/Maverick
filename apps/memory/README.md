...
- `memory_ingest_source` provides generic app-owned source ingestion for `inline_markdown`, bounded local `storage_file` text sources, remote `remote_storage_file` payloads produced by Storage `drive_index`, and `app_entity` reference snapshots from another app's official reference summarize/resolve surfaces. Local Storage file ingestion accepts a stable Storage file id or a workspace-relative Storage path and resolves metadata and content through official Storage `reference_resolve`, `file_info`, `preview_text`, and `read_file` surfaces before Memory writes its own verified source/chunk content store; when Storage exposes a file SHA-256, Memory records the source version as `file_bytes` while keeping the canonical body hash on the stored content. Remote Storage file ingestion reuses the same validation and materialization path as the transitional `memory_ingest_storage_source` action. App-entity ingestion never reads the owner app's private database; it stores a redaction-safe reference snapshot with the owning app id, entity type, entity id, and deep link as source locator. When compiled, linked app-owned sources can produce chunk-level citations just like Storage-backed sources when they include credible extracted-reference text.
...
Remote Storage ingest is idempotent by stable Storage file id when no target node is provided, and idempotent by `(node_id, stable_storage_file_id)` when a target node is provided. This lets one Drive file support multiple Memory nodes while avoiding duplicate refs on the same node. It requires a Storage `source_version` so source versions and citations remain stable. A successful ingest clears prior Storage stale metadata for that file reference and materializes the matching source document, source version, content-store body, and source chunks before compile. When `compile_after_ingest=true`, Memory also compiles immediately into claims, citations, and a compiled wiki page. If Storage preview/export is unavailable and no bounded `preview_text` was supplied, ingest fails without creating a fresh source version or fresh wiki page.

The first v3 implementation keeps existing Drive-to-Memory behavior intact while adding the migration, content-store foundation, deterministic multi-chunk source extraction, chunk-level citation support, app-owned source chunk FTS, read-only source/chunk retrieval primitives, source chunk matches in node retrieval, an app-owned job lifecycle with `run_next` plus bounded `run_until_idle`, and `memory_ingest_source` for `inline_markdown`, local workspace `storage_file` text sources, remote Storage file payloads fetched through Storage-owned file surfaces, and app-entity reference snapshots. Memory currently reaches Storage and app reference surfaces through the platform-hosted `maverick app ... mcp` wrapper; a future direct app-to-app SDK can replace that transport without changing Memory's stored source model. Embeddings remain future work on top of the schema and verified content paths now declared by the app.
