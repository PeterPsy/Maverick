...
- `memory_ingest_source` provides generic app-owned source ingestion for `inline_markdown`, bounded local `storage_file` text sources, and remote `remote_storage_file` payloads produced by Storage `drive_index`. Local Storage file ingestion accepts a stable Storage file id or a workspace-relative Storage path and resolves metadata and content through official Storage `reference_resolve`, `file_info`, `preview_text`, and `read_file` surfaces before Memory writes its own verified source/chunk content store; when Storage exposes a file SHA-256, Memory records the source version as `file_bytes` while keeping the canonical body hash on the stored content. Remote Storage file ingestion reuses the same validation and materialization path as the transitional `memory_ingest_storage_source` action. When compiled, those linked app-owned sources can produce chunk-level citations just like Storage-backed sources.
...
- `search` covers nodes plus compiled wiki page, claim text, and linked source chunks. `context` and `search` include `match_sources` plus `source_chunk_matches` when verified chunks contributed to retrieval. `wiki_query` returns wiki-page and claim matches directly for agents. `source_query` searches verified source chunks through Memory's app-owned source chunk FTS and marks older-version chunk matches as `stale`, `fetch_chunks` hydrates up to 20 chunks with content-store SHA verification, and both chunk surfaces include normalized chunk identity, freshness, locators, and citations when available. `inspect_source` shows a source document with versions, chunks, linked nodes, and recent ingest jobs. `context`, `search`, and `wiki_query` include normalized `storage_references` with stable Storage file ids, Drive ids, source versions, deep links, and preview/export request arguments when available.
...
The first v3 implementation keeps existing Drive-to-Memory behavior intact while adding the migration, content-store foundation, deterministic multi-chunk source extraction, chunk-level citation support, app-owned source chunk FTS, read-only source/chunk retrieval primitives, source chunk matches in node retrieval, a minimal app-owned job lifecycle with `run_next`, and `memory_ingest_source` for `inline_markdown`, local workspace `storage_file` text sources, and remote Storage file payloads fetched through Storage-owned file surfaces. App-entity ingestion and embeddings are future work on top of the schema and verified content paths now declared by the app.
