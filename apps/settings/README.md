...
- The platform settings panel manages installation-published direct agentic profiles through workspace bindings. Platform and workspace admins can attach an eligible credential binding, choose the workspace default, restrict actors, tools, confirmations, and cost, and inspect Core-owned egress/data policy, pinned engine/adapter/provider/model, routing, health and effective capabilities. The browser does not classify session data, patch allowed remote-data classes or add capabilities, and Settings never rewrites existing sessions. Hosted text selection remains an independent `plain_hosted_chat`/`fast_model` path. Speech-to-text settings expose Deepgram audio transcription and conversation models as separate choices because file/one-shot transcription uses Nova-3 over `/v1/listen`, while realtime conversation uses Flux models over the WebSocket v2 Listen API.
- Settings shows provider/upstream, data destination, effective egress policy and Core-classified data set, collection/ZDR policy, read-only attestation state/scope/revision, binding/profile state and effective capabilities. Contained remote controls remain blocked by the server-owned posture; the effective snapshot is read-only and never becomes a client capability override. No fake-data consent, data-class checkbox, credential authority or client capability override exists. Quarantined runtime rows expose `recovery_required` with only an allowlisted public reason instead of arbitrary Core diagnostic detail.
