...
- Phase-0 remote agentic release is visibly **NO-GO**. Settings keeps Google, OpenRouter, and future hosted-agentic definitions visible even after suspension and shows provider/upstream, binding enabled/disabled state, profile status, certificate revoked/effective status, and containment eligibility. A contained disabled binding cannot be enabled from the browser; an already-enabled one can only be narrowed/disabled. Settings sends neither fake-data consent nor `workspace_internal_fake`. Codex agentic and hosted text controls are unchanged. Quarantined runtime rows expose `recovery_required` and its redaction-safe reason instead of a generic runtime error.
