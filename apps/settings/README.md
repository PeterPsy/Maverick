...
- The platform settings panel shows Codex runtime model selection separately from hosted text model selection. Codex remains the agentic provider for tools, filesystem, MCP, and skills; hosted text providers such as OpenRouter govern only `plain_hosted_chat` and `fast_model`. Speech-to-text settings expose Deepgram audio transcription and conversation models as separate choices because file/one-shot transcription uses Nova-3 over `/v1/listen`, while realtime conversation uses Flux models over the WebSocket v2 Listen API.
