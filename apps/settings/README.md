...
- The platform settings panel manages installation-published agentic definitions through workspace bindings. Platform and workspace admins can attach an eligible credential binding, choose the workspace default, restrict actors, tools, egress classes, confirmations, and cost, and inspect the pinned engine/adapter/provider/model, certificate status and expiry, routing, health, and preview posture. Settings never edits a `certified` boolean and never rewrites existing sessions. Hosted text selection remains an independent `plain_hosted_chat`/`fast_model` path. Speech-to-text settings expose Deepgram audio transcription and conversation models as separate choices because file/one-shot transcription uses Nova-3 over `/v1/listen`, while realtime conversation uses Flux models over the WebSocket v2 Listen API.
- Remote agentic bindings are visibly restricted to preview use with synthetic data. Enabling one requires an explicit fake-data workspace declaration, an active workspace-authorized credential binding, a live certificate, and a workspace policy that retains the synthetic-data egress class.
...
- The platform settings page also loads core-metered workspace token history from the admin-only `GET /api/usage/timeseries` surface. It renders 24 gap-filled hourly buckets and 30 daily buckets with category totals and an explicit coverage start. These charts describe locally observed runtime consumption across direct and delegated sessions; they remain separate from provider subscription percentages and do not claim coverage before metering was enabled.
