...
Use this skill when creating a new Maverick app from scratch.

Keep app work contract-first, app-owned, and core-agnostic.

## Mandatory Pairing

Also use `maverick-code-skill` for any task that touches Maverick source or workspace app files.
...
1. Generate the app with `maverick core cli run core.app-sdk.create --app-id <app_id> --template-id <template_id> --json`.
...
4. Validate with `maverick core cli run core.app-sdk.validate --app-id <app_id> --json`.
5. Register with `maverick core cli run core.app-sdk.register-local --app-id <app_id> --json`.
6. Install with `maverick core cli run core.app-sdk.install-local --app-id <app_id> --json`.
7. Verify with `maverick core cli run core.app-sdk.status --app-id <app_id> --json`, `/api/status`, `/api/apps`, mounted frontend/backend checks, and CLI/MCP listings when declared.
8. Package with `maverick core cli run core.app-sdk.package --app-id <app_id> --json` when a generated artifact is useful.

Do not leave SDK scaffold behavior in place when the user asked for a real app. The SDK gives the correct Maverick app shape; the app still needs real domain behavior, UI, storage, tests, and smoke verification.
...
Do not copy FastAPI route patterns into Maverick apps unless the app host explicitly supports that surface.
...
- SDK-generated source validates with `maverick core cli run core.app-sdk.validate --app-id <app_id> --json` when the SDK is used
...
- workspace-local app registers with `maverick core cli run core.app-sdk.register-local --app-id <app_id> --json` when using the SDK flow
...
- workspace-local app installs with `maverick core cli run core.app-sdk.install-local --app-id <app_id> --json` when using the SDK flow
...
- frontend build output exists when declared, and frontend apps are refreshed through `maverick app <app_id> frontend build --json`
...
A new Maverick app is done when:
