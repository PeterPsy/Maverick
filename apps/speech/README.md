...
- Synthesis uses the selected TTS engine from workspace settings. `auto` prefers Piper when a local model is configured, then falls back to `espeak-ng` or `espeak`. `espeak` remains a diagnostic fallback rather than the preferred user voice; `kokoro-openrouter` and `kokoro-deepinfra` are explicit governed remote choices. The latter uses DeepInfra's dedicated streaming endpoint and requires a separately governed `deepinfra-api-key`; it is never selected automatically when that credential is absent. Speech owns language-to-voice selection for every engine: an explicit workspace `synthesis_language` such as `it-IT` overrides per-request hints and inference, while `auto` uses a caller hint and then conservative text inference. Kokoro keeps `af_heart` as its English fallback and selects `if_sara` for Italian; Piper selects the configured voice whose BCP-47 language best matches.
...
- Flux dictation sessions persist only metadata and transcript state, but Speech serializes requests by `session_id` through bounded bucketed lock files and enforces consecutive `chunk_index` values, the same inline 20 MB and 180 second budgets across the whole session, plus a bounded chunk count before sending any chunk to the provider. A final dictation request may be close-only with an empty body so the backend can flush and close the Deepgram stream even when the browser's final `dataavailable` event is empty or too small to transcribe as standalone audio.
- Workspace settings may select the preferred synthesis engine, synthesis language (`auto` or a BCP-47 tag), transcription engine, and transcription profile (`fast`, `balanced`, or `accurate`). Host paths, model choices, device choices, and compute settings remain operator configuration, not workspace data.
- Piper support is local-only and requires `piper` on `PATH` plus `MAVERICK_SPEECH_PIPER_MODEL` pointing at a local voice model. Optional redacted voice metadata can be supplied with `MAVERICK_SPEECH_PIPER_VOICE_ID`, `MAVERICK_SPEECH_PIPER_LANGUAGE`, and `MAVERICK_SPEECH_PIPER_VOICES_JSON`. Cache misses use a workspace-scoped persistent Piper worker in `auto` mode when the `piper-tts` Python package is available, so repeated Chat TTS chunks reuse the loaded voice instead of spawning the `piper` binary per chunk. `prewarm_synthesis_worker` loads the selected workspace language voice without synthesizing placeholder text; Chat calls it automatically when capabilities advertise support, and CLI callers may invoke it explicitly. The worker prepares WAV channel, sample width, and sample rate parameters before synthesis so Piper voices that write frames directly do not fail with missing channel metadata. Set `MAVERICK_SPEECH_PIPER_WORKER=persistent` to fail visibly instead of falling back to the subprocess path, or `entrypoint` to force per-request subprocess synthesis.
...
- Buffered synthesis responses return bounded inline audio as `audio_base64` plus `content_type`, the selected `language` and `voice`, and redaction-safe `engine_seconds`/`request_total_seconds` metrics without storing input text. Local engines return WAV audio with `retention: derived_cache`; repeated identical local synthesis requests reuse workspace-scoped cached WAV bytes under `data/speech/cache/tts/`. Kokoro retains MP3 as the buffered compatibility response and does not cache provider output. When a backend consumer sends `response_mode: "stream"` and `format: "pcm"`, either governed Kokoro route instead returns raw 24 kHz, mono, signed 16-bit little-endian PCM through the core's progressive backend protocol. The persistent Speech worker owns bounded provider-specific keep-alive pools, sends response metadata as soon as upstream headers arrive, relays each available body chunk, and closes the active upstream connection when the browser disconnects. A downstream disconnect cancels the provider stream, records the job as incomplete, and treats concurrent socket-close read failures as normal cancellation rather than provider failure. Credentials remain inside ephemeral Core-to-app secret delivery. Stream response headers expose only `X-Generation-Id`, the PCM description, and already-known `Server-Timing` phases; completed or interrupted stream jobs persist redaction-safe `backend_entrypoint_ms`, upstream connect/headers/first-byte/last-byte timings, connection reuse, byte count, completion state, and a request total that includes the first-byte wait. Browser playback observations are correlated by generation id and persisted as separate metadata-only jobs without text or audio; they include the bounded Web Audio context state and Audio Session category so silent Apple-mobile playback can be distinguished from upstream latency. The local cache key includes engine binary/model/config file fingerprints, small file hashes, and optional operator-supplied Piper model/config digests so voice changes do not silently serve stale audio. Age-based cache eviction is opportunistic and bounded by a cleanup marker, while every cache miss still enforces the count and byte caps so chunked playback bursts cannot grow the cache past its configured limits. Oversize generated audio is rejected before cache write. Generated audio artifacts are not written under workspace storage and only capped job metadata is persisted.
...
CLI exposes workspace engine/language configuration, engine inspection, persistent worker status, and Storage-file transcription:
...
maverick app speech cli run speech --action set_engine --synthesis-engine piper --synthesis-language it-IT
maverick app speech cli run speech --action worker_status
maverick app speech cli run speech --action prewarm_synthesis_worker
...
## Kokoro Latency Benchmark

Use the app-owned harness to separate first-byte latency from full audio generation on either OpenRouter or DeepInfra. Its default matrix covers 40, 80, 120, and 450 characters, MP3 and PCM, and fresh versus pooled HTTPS connections. One hundred requests are distributed round-robin with a short interval, and the summary reports p50/p90 connect, headers, first audio byte, last audio byte, and total time for every case.

Inspect the matrix without making a provider request:

```bash
python3 apps/speech/scripts/benchmark_kokoro.py --dry-run
```

Run the benchmark with an operator-provided test credential. The script reads the credential only from the process environment and does not write it or the input text to workspace data:

```bash
OPENROUTER_API_KEY=... python3 apps/speech/scripts/benchmark_kokoro.py --requests 100
DEEPINFRA_API_KEY=... python3 apps/speech/scripts/benchmark_kokoro.py --provider deepinfra --requests 100 --language it --voice if_sara
```

Use `--include-requests` when individual generation ids and timings are needed for diagnosis. The production path still receives its credential from Core Secrets; the environment variable is limited to this standalone benchmark harness.
