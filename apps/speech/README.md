...
- Flux dictation sessions persist only metadata and transcript state, but Speech enforces the same inline 20 MB and 180 second budgets across the whole `session_id`, plus a bounded chunk count, before sending any chunk to the provider. A final dictation request may be close-only with an empty body so the backend can flush and close the Deepgram stream even when the browser's final `dataavailable` event is empty or too small to transcribe as standalone audio.
