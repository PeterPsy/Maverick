...
- Synthesis uses the selected TTS engine from workspace settings. `auto` prefers Piper when a local model is configured, then falls back to `espeak-ng` or `espeak`. `espeak` remains a diagnostic fallback rather than the preferred user voice; `kokoro-openrouter` is an explicit governed remote choice. Speech owns language-to-voice selection for every engine: an explicit workspace `synthesis_language` such as `it-IT` overrides per-request hints and inference, while `auto` uses a caller hint and then conservative text inference. Kokoro keeps `af_heart` as its English fallback and selects `if_sara` for Italian; Piper selects the configured voice whose BCP-47 language best matches.
...
Use the app-owned harness to separate OpenRouter first-byte latency from full audio generation. Its default matrix covers 40, 80, 120, and 450 characters, MP3 and PCM, and fresh versus pooled HTTPS connections. One hundred requests are distributed round-robin with a short interval, and the summary reports p50/p90 connect, headers, first audio byte, last audio byte, and total time for every case.
