...
- Remote speech providers such as Deepgram, Cartesia, and Kokoro-hosted are deferred governed providers, not app-local engines in this slice. When cloud speech support is added, Speech must consume provider capability and routing decisions through official core/app surfaces, receive ephemeral secret delivery only through Core Secrets grants or provider bindings, and persist only redaction-safe metadata. Speech must not import core internals, bypass the provider router, or read raw provider credentials from workspace data.
