...
When Core invokes Speech as the selected `speech.transcription` provider, it passes a redaction-safe `provider_config.speech_stt` payload. Deepgram `transcribe_file` and one-shot `transcribe_audio` resolve the configured `audio_transcription_model_id` from that payload and call `https://api.deepgram.com/v1/listen?model=<audio-model>`. `transcribe_audio` requests with `conversation: true` and a `session_id` resolve the configured `conversation_model_id`, open a Deepgram Flux Listen v2 WebSocket, send audio chunks over that live session, and return transcript plus turn events. Flux streaming requires the persistent Speech backend worker because the WebSocket session is process-owned; when the worker is disabled, Speech does not advertise streaming support.
