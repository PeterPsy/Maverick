...
- Governed remote provider support is limited to Deepgram STT and Kokoro through OpenRouter TTS. Speech receives ephemeral secret delivery only through Core Secrets grants, persists only redaction-safe metadata, and does not store raw provider credentials in `data/speech`.
