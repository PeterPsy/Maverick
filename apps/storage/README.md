...
- Users can create folders inside `storage/uploaded/` and `storage/generated/` from the sidebar's selected folder, and drag Storage files into visible folder cards or sidebar tree folders. File moves stay inside the file's current storage role.
