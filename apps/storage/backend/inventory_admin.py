...
def catalog_directories(data_root: Path, uploaded_root: Path, generated_root: Path, *,
                        role: str, parent: str, offset: int, limit: int, dataset_revision: int | None, query: str = '') -> dict:
    with storage_mutation_lock(data_root, shared=True):
        if uses_sqlite(data_root):
            ...
            with index.transaction() as connection:
                ...
                total, entries = page['pagination']['total'], page['folders']
            ...
            folders = [entry for entry in folders if (role == 'all' or entry['role'] == role)
                and ((query.casefold() in (entry['name'] + ' ' + entry['workspace_relative_path']).casefold()) if query
                    else entry['relative_path'].rpartition('/')[0] == parent)]
