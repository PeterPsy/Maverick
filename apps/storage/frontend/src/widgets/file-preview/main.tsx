...
function StorageFilePreviewWidget() {
  ...
  useEffect(() => {
    function syncFullscreenState() {
      if (elementIsFullscreen(rootRef.current)) {
  ...
  async function toggleFullscreen() {
    ...
    if (!root) return;
    if (canRequestFullscreen(root)) {
      try {
        await requestElementFullscreen(root);
        setFullscreenMode('native');
        return;
      } catch {
        // Fall through when the host iframe denies native fullscreen.
      }
    }
  ...
  if (!file || previewLoading) {
    return <FileWidgetSkeleton rootRef={rootRef} />;
  }

  return (
    <main className={`file-widget ${fullscreenActive ? 'is-fullscreen' : ''}`} ref={rootRef}>
      <header className="file-widget__bar">
        <h3>
        ...
        <div className="file-widget__actions">
          <button
            className="file-widget__action"
            type="button"
            onClick={() => toggleFullscreen().catch((fullscreenError: Error) => setError(fullscreenError.message))}
            aria-label={fullscreenActive ? 'Exit full screen' : 'Open preview full screen'}
            aria-pressed={fullscreenActive}
            title={fullscreenActive ? 'Exit full screen' : 'Open preview full screen'}
          >
            <Icon name={fullscreenActive ? 'fullscreen_exit' : 'fullscreen'} />
          </button>
          <button
            className="file-widget__action file-widget__open"
            type="button"
