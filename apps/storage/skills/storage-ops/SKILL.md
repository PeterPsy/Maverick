...
- CLI-only `upload_local_file`: upload a trusted local source path into Storage through chunked local upload sessions without putting large base64 payloads on the shell command line. Sandbox invocations may read only source paths inside the workspace root; full-access operator invocations may read other local paths.
...
maverick app storage cli run storage --arguments-json '{
  ...
  "source_path": "/path/to/workspace/runtime/output.pdf",
