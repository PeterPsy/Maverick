"""Tests for the Storage file preview widget mount."""
...
class StorageWidgetTestCase(unittest.TestCase):
    ...
    def test_platform_exposes_storage_file_preview_widget(self) -> None:
        ...
        registry_status, registry_payload, _registry_headers = self.invoke(
            ...
            query_string="host=chat&content_kind=storage.file.preview",
            cookie=cookie,
        )
        ...
        self.assertEqual(registry_payload["items"][0]["owner_app_id"], "storage")
        self.assertEqual(registry_payload["items"][0]["widget_id"], "file-preview")
        self.assertEqual(registry_payload["items"][0]["frontend_mount"], "/api/apps/widgets/storage/file-preview/frontend/")
        self.assertEqual(mount_status, 200)
        self.assertIn("text/html", mount_headers["Content-Type"])
        self.assertIn(b"Storage file preview", mount_payload)
    ...
    def test_platform_exposes_storage_sidebar_widget(self) -> None:
        ...
        self.assertEqual(widgets["storage-sidebar"]["owner_app_id"], "storage")
        self.assertEqual(widgets["storage-sidebar"]["frontend_mount"], "/api/apps/widgets/storage/storage-sidebar/frontend/")
        self.assertEqual(mount_status, 200)
        self.assertIn("text/html", mount_headers["Content-Type"])
        self.assertIn(b"Storage Sidebar Widget", mount_payload)
