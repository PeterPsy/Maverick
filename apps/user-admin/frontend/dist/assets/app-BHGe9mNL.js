(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))n(t);new MutationObserver(t=>{for(const i of t)if(i.type==="childList")for(const c of i.addedNodes)c.tagName==="LINK"&&c.rel==="modulepreload"&&n(c)}).observe(document,{childList:!0,subtree:!0});function s(t){const i={};return t.integrity&&(i.integrity=t.integrity),t.referrerPolicy&&(i.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?i.credentials="include":t.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function n(t){if(t.ep)return;t.ep=!0;const i=s(t);fetch(t.href,i)}})();let p=[],m=[],u=[],l="";async function o(a,e={}){const s=await fetch(a,{credentials:"same-origin",headers:{"Content-Type":"application/json",...e.headers||{}},...e}),n=await s.json();if(!s.ok)throw new Error(n.detail||n.error||`Request failed ${s.status}`);return n}function y(){return p.find(a=>a.user_id===l)||p[0]}function h(a,e){return a.memberships.find(s=>s.workspace_id===e)}async function d(){const[a,e,s]=await Promise.all([o("/api/admin/users"),o("/api/admin/workspaces"),o("/api/admin/workspace-apps")]);p=a.items,m=e.items,u=s.items,(!l||!p.some(n=>n.user_id===l))&&(l=p[0]?.user_id||""),b()}async function w(a){const e=new FormData(a),s={username:String(e.get("username")||""),password:String(e.get("password")||""),display_name:String(e.get("display_name")||""),email:String(e.get("email")||""),platform_role:String(e.get("platform_role")||"member")};l=(await o("/api/admin/users",{method:"POST",body:JSON.stringify(s)})).user_id,a.reset(),await d()}async function g(a,e){const s=new FormData(a);await o(`/api/admin/users/${encodeURIComponent(e.user_id)}`,{method:"PATCH",body:JSON.stringify({display_name:String(s.get("display_name")||""),email:String(s.get("email")||""),platform_role:String(s.get("platform_role")||"member"),account_type:String(s.get("account_type")||"standard"),is_active:s.get("is_active")==="on"})}),await d()}async function v(a,e){const s=new FormData(a),n=String(s.get("password")||""),t=String(s.get("password_confirmation")||"");if(n!==t)throw new Error("Le password non coincidono");await o(`/api/admin/users/${encodeURIComponent(e.user_id)}/password`,{method:"POST",body:JSON.stringify({password:n})}),a.reset(),window.alert("Password aggiornata")}async function _(a){const e=a.display_name||a.username;window.confirm(`Eliminare definitivamente ${e}? Verranno rimossi accesso, sessioni e assegnazioni workspace.`)&&(await o(`/api/admin/users/${encodeURIComponent(a.user_id)}`,{method:"DELETE"}),l="",await d())}async function k(a){const e=m.map(s=>{const n=document.querySelector(`[data-workspace-enabled="${s.workspace_id}"]`),t=document.querySelector(`[data-workspace-role="${s.workspace_id}"]`);return n?.checked?{workspace_id:s.workspace_id,role:t?.value||"member"}:null}).filter(Boolean);await o(`/api/admin/users/${encodeURIComponent(a.user_id)}/workspaces`,{method:"PUT",body:JSON.stringify({memberships:e})}),await d()}async function $(a){await o(`/api/admin/workspace-apps/${encodeURIComponent(a.workspace_id)}/${encodeURIComponent(a.app_id)}`,{method:"POST",body:JSON.stringify({source_id:a.source_id,enabled:!0})}),await d()}async function S(a,e){await o(`/api/admin/workspace-apps/${encodeURIComponent(a.workspace_id)}/${encodeURIComponent(a.app_id)}`,{method:"PATCH",body:JSON.stringify({status:e?"enabled":"disabled"})}),await d()}async function E(a){await o(`/api/admin/workspace-apps/${encodeURIComponent(a.workspace_id)}/${encodeURIComponent(a.app_id)}`,{method:"DELETE",body:JSON.stringify({})}),await d()}function I(){return p.map(a=>{const e=a.user_id===y()?.user_id?"is-active":"",s=a.platform_role==="admin"?"Admin":"Member";return`<button class="ua-user ${e}" data-user-id="${a.user_id}">
      ...
      </label>`}).join("")}function U(){return m.map(a=>{const e=u.filter(t=>t.workspace_id===a.workspace_id),s=e.filter(t=>t.status==="enabled").length,n=e.filter(t=>t.installed).length;return`<details class="ua-app-workspace">
        ...
        <div class="ua-apps">
          ${e.map(t=>{const i=t.status==="enabled",c=t.installed,f=c?t.status:"non installata";return`<div class="ua-app-row">
      ...
      <div class="ua-content">
        ...
        ${e?`<div class="ua-profile-row">
          ...
          <details class="ua-card ua-collapsible" open>
            ...
            <div class="ua-app-workspaces">${U()}</div>
          </details>`:'<section class="ua-card"><h2>Nessun utente</h2></section>'}
      </div>
    </section>
  </section>`,L())}function L(){document.querySelectorAll("[data-user-id]").forEach(e=>{e.addEventListener("click",()=>{l=e.dataset.userId||"",b()})}),document.getElementById("create-user")?.addEventListener("submit",e=>{e.preventDefault(),w(e.currentTarget).catch(r)});const a=y();document.getElementById("edit-user")?.addEventListener("submit",e=>{e.preventDefault(),a&&g(e.currentTarget,a).catch(r)}),document.getElementById("reset-password")?.addEventListener("submit",e=>{e.preventDefault(),a&&v(e.currentTarget,a).catch(r)}),document.getElementById("delete-user")?.addEventListener("click",()=>{a&&_(a).catch(r)}),document.getElementById("save-memberships")?.addEventListener("click",()=>{a&&k(a).catch(r)}),document.querySelectorAll("[data-app-toggle]").forEach(e=>{e.addEventListener("change",()=>{const s=u.find(n=>`${n.workspace_id}:${n.app_id}`===e.dataset.appToggle);s&&S(s,e.checked).catch(r)})}),document.querySelectorAll("[data-app-install]").forEach(e=>{e.addEventListener("click",()=>{const s=u.find(n=>`${n.workspace_id}:${n.app_id}`===e.dataset.appInstall);s&&$(s).catch(r)})}),document.querySelectorAll("[data-app-uninstall]").forEach(e=>{e.addEventListener("click",()=>{const s=u.find(n=>`${n.workspace_id}:${n.app_id}`===e.dataset.appUninstall);s&&E(s).catch(r)})})}function r(a){const e=a instanceof Error?a.message:"Errore inatteso";window.alert(e)}d().catch(r);
