(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))n(t);new MutationObserver(t=>{for(const i of t)if(i.type==="childList")for(const c of i.addedNodes)c.tagName==="LINK"&&c.rel==="modulepreload"&&n(c)}).observe(document,{childList:!0,subtree:!0});function s(t){const i={};return t.integrity&&(i.integrity=t.integrity),t.referrerPolicy&&(i.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?i.credentials="include":t.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function n(t){if(t.ep)return;t.ep=!0;const i=s(t);fetch(t.href,i)}})();let d=[],m=[],u=[],p="";async function o(a,e={}){const s=await fetch(a,{credentials:"same-origin",headers:{"Content-Type":"application/json",...e.headers||{}},...e}),n=await s.json();if(!s.ok)throw new Error(n.detail||n.error||`Request failed ${s.status}`);return n}function b(){return d.find(a=>a.user_id===p)||d[0]}function f(a,e){return a.memberships.find(s=>s.workspace_id===e)}async function l(){const[a,e,s]=await Promise.all([o("/api/admin/users"),o("/api/admin/workspaces"),o("/api/admin/workspace-apps")]);d=a.items,m=e.items,u=s.items,(!p||!d.some(n=>n.user_id===p))&&(p=d[0]?.user_id||""),y()}async function _(a){const e=new FormData(a),s={username:String(e.get("username")||""),password:String(e.get("password")||""),display_name:String(e.get("display_name")||""),email:String(e.get("email")||""),platform_role:String(e.get("platform_role")||"member")};p=(await o("/api/admin/users",{method:"POST",body:JSON.stringify(s)})).user_id,a.reset(),await l()}async function g(a,e){const s=new FormData(a);await o(`/api/admin/users/${encodeURIComponent(e.user_id)}`,{method:"PATCH",body:JSON.stringify({display_name:String(s.get("display_name")||""),email:String(s.get("email")||""),platform_role:String(s.get("platform_role")||"member"),account_type:String(s.get("account_type")||"standard"),is_active:s.get("is_active")==="on"})}),await l()}async function v(a){const e=m.map(s=>{const n=document.querySelector(`[data-workspace-enabled="${s.workspace_id}"]`),t=document.querySelector(`[data-workspace-role="${s.workspace_id}"]`);return n?.checked?{workspace_id:s.workspace_id,role:t?.value||"member"}:null}).filter(Boolean);await o(`/api/admin/users/${encodeURIComponent(a.user_id)}/workspaces`,{method:"PUT",body:JSON.stringify({memberships:e})}),await l()}async function w(a){await o(`/api/admin/workspace-apps/${encodeURIComponent(a.workspace_id)}/${encodeURIComponent(a.app_id)}`,{method:"POST",body:JSON.stringify({source_id:a.source_id,enabled:!0})}),await l()}async function k(a,e){await o(`/api/admin/workspace-apps/${encodeURIComponent(a.workspace_id)}/${encodeURIComponent(a.app_id)}`,{method:"PATCH",body:JSON.stringify({status:e?"enabled":"disabled"})}),await l()}async function $(a){await o(`/api/admin/workspace-apps/${encodeURIComponent(a.workspace_id)}/${encodeURIComponent(a.app_id)}`,{method:"DELETE",body:JSON.stringify({})}),await l()}function S(){return d.map(a=>{const e=a.user_id===b()?.user_id?"is-active":"",s=a.platform_role==="admin"?"Admin":"Member";return`<button class="ua-user ${e}" data-user-id="${a.user_id}">
      ...
      </button>`}).join("")}function E(a){return m.map(e=>{const s=f(a,e.workspace_id);return`<label class="ua-membership">
      ...
      </label>`}).join("")}function A(){return m.map(a=>{const e=u.filter(t=>t.workspace_id===a.workspace_id),s=e.filter(t=>t.status==="enabled").length,n=e.filter(t=>t.installed).length;return`<details class="ua-app-workspace">
        ...
        <div class="ua-apps">
          ${e.map(t=>{const i=t.status==="enabled",c=t.installed,h=c?t.status:"non installata";return`<div class="ua-app-row">
                <span class="ua-app-icon material-symbols-rounded" aria-hidden="true">${i?"apps":"hide_source"}</span>
                <span class="ua-app-copy">
                  <strong>${t.name}</strong>
                  <small>${t.app_id} · v${t.version} · ${h}</small>
                </span>
                ${c?`<label class="ua-switch">
      ...
      </details>`}).join("")}function y(){const a=document.getElementById("app"),e=b();a&&(a.innerHTML=`<section class="ua-shell">
    <aside class="ua-rail">
      <div>
    ...
    <section class="ua-main">
      ...
      ${e?`<form class="ua-card ua-detail" id="edit-user">
          ...
          <section class="ua-card">
            ...
            <div class="ua-memberships">${E(e)}</div>
          </section>
          <details class="ua-card ua-collapsible" open>
            ...
            <div class="ua-app-workspaces">${A()}</div>
          </details>`:'<section class="ua-card"><h2>Nessun utente</h2></section>'}
    </section>
  </section>`,I())}function I(){document.querySelectorAll("[data-user-id]").forEach(e=>{e.addEventListener("click",()=>{p=e.dataset.userId||"",y()})}),document.getElementById("create-user")?.addEventListener("submit",e=>{e.preventDefault(),_(e.currentTarget).catch(r)});const a=b();document.getElementById("edit-user")?.addEventListener("submit",e=>{e.preventDefault(),a&&g(e.currentTarget,a).catch(r)}),document.getElementById("save-memberships")?.addEventListener("click",()=>{a&&v(a).catch(r)}),document.querySelectorAll("[data-app-toggle]").forEach(e=>{e.addEventListener("change",()=>{const s=u.find(n=>`${n.workspace_id}:${n.app_id}`===e.dataset.appToggle);s&&k(s,e.checked).catch(r)})}),document.querySelectorAll("[data-app-install]").forEach(e=>{e.addEventListener("click",()=>{const s=u.find(n=>`${n.workspace_id}:${n.app_id}`===e.dataset.appInstall);s&&w(s).catch(r)})}),document.querySelectorAll("[data-app-uninstall]").forEach(e=>{e.addEventListener("click",()=>{const s=u.find(n=>`${n.workspace_id}:${n.app_id}`===e.dataset.appUninstall);s&&$(s).catch(r)})})}function r(a){const e=a instanceof Error?a.message:"Errore inatteso";window.alert(e)}l().catch(r);
