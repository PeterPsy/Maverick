...
""",
    ...
    def test_workspace_snapshot_cache_is_parent_mediated_without_legacy_prepaint(self) -> None:
        ...
        self.assertNotIn("migrationSeed", api_source)
        self.assertIn("purgeLegacyWorkspaceSnapshots();", api_source)
        self.assertNotIn("sessionStorage.getItem", api_source)
        self.assertNotIn("legacySnapshotRevision", api_source)
    ...
    def test_frontend_refreshes_on_app_data_changed_events(self) -> None:
        ...
        cleanup_source = (APP_ROOT / "frontend" / "src" / "legacySnapshotCleanup.ts").read_text(encoding="utf-8")
        self.assertIn("purgeLegacyWorkspaceSnapshots();", api_source)
        self.assertIn("sessionStorage.removeItem(key)", cleanup_source)
        self.assertNotIn("getItem", cleanup_source)
