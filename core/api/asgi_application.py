...
class PlatformAsgiHost:
    ...
    def __init__(self, state: PlatformState | None = None) -> None:
        self.state = state or bootstrap_platform_state()
        self.http_host = PlatformHost(self.state, start_path=self.state.repository_root)
    ...
    async def _handle_websocket(self, scope: dict[str, Any], receive: AsgiReceive, send: AsgiSend) -> None:
        ...
        if not path.startswith(RUNTIME_SESSION_WS_PREFIX):
            ...
            return
        await stream_runtime_session_events(state=self.state, scope=scope, receive=receive, send=send)
    ...
    async def _handle_http(self, scope: dict[str, Any], receive: AsgiReceive, send: AsgiSend) -> None:
        ...
        response_chunks = self.http_host(_wsgi_environ(scope, body), start_response)
