...
def session_cookie_header(session_id: str, *, expires_at: datetime | None = None) -> tuple[str, str]:
...
def handle_session_api(state: PlatformState, environ: dict, start_response: StartResponse) -> list[bytes] | None:
    ...
    if path == "/api/auth/login" and method == "POST":
        ...
        return json_response(
            ...
            headers=[session_cookie_header(session.session_id, expires_at=expires_at)],
        )
    if path == "/api/auth/logout" and method == "POST":
        context = resolve_request_session(state, environ)
        if context is not None:
            revoke_auth_session(state.identity_store, session=context.session)
        return json_response(start_response, {"authenticated": False}, headers=[clear_session_cookie_header()])
    return None
