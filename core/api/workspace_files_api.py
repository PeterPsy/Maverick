...
def handle_workspace_files_api(state: PlatformState, environ: dict, start_response: StartResponse, *, start_path) -> list[bytes] | None:
    ...
    try:
        uploaded = save_workspace_upload(
            ...
            max_storage_bytes=state.workspace_store.get_quota(context.workspace_id).max_storage_bytes,
            start_path=start_path,
        )
    except ValueError as error:
        if str(error) == "upload_too_large":
            return json_response(start_response, {"error": "upload_too_large"}, status="413 Payload Too Large")
        if str(error) == "workspace_storage_quota_exceeded":
            return json_response(
                start_response,
                {"error": "workspace_storage_quota_exceeded"},
