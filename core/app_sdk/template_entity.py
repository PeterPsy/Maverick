...
<html><head><meta charset="utf-8"><title>{request.name or title_from_slug(request.app_id)}</title></head>
<body><main><h1>{request.name or title_from_slug(request.app_id)}</h1><p>Entity app ready. Rebuild frontend for full React UI.</p></main></body></html>
"""
...
from store import create_record, get_record, list_records, reference_manifest, search_records


MUTATING_ACTIONS = {{"create"}}
...
def handle_action(data_root: Path, payload: dict[str, object]) -> tuple[int, dict[str, object]]:
    ...
    try:
        ...
        if action == "get":
            return 200, {{"item": get_record(data_root, str(payload.get("entity_type") or "{entities[0]}"), str(payload.get("id") or ""))}}
        if action == "search":
...
def app_events_for_action(action: str) -> list[dict[str, str]]:
    if action not in MUTATING_ACTIONS:
        return []
    return [{{"type": "maverick.app.data-changed", "owner_app_id": "{request.app_id}", "resource": "records"}}]
'''
...
def _mcp_entrypoint(request: AppSdkCreateRequest, entities: list[str]) -> str:
    mapping = {
        f"{request.app_id}_reference_manifest": "reference_manifest",
        f"{request.app_id}_list": "list",
        f"{request.app_id}_create": "create",
        f"{request.app_id}_get": "get",
        f"{request.app_id}_search": "search",
    }
