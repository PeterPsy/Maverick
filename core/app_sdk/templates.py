...
def _contract_payload(request: AppSdkCreateRequest) -> dict[str, Any]:
    ...
    has_buildable_frontend = has_frontend
...
def _frontend_files(request: AppSdkCreateRequest) -> dict[str, str]:
    return react_vite_files(request)
...
```bash
maverick core cli run core.app-sdk.validate --app-id {request.app_id} --json
```

## Workspace-Local Install

```bash
maverick core cli run core.app-sdk.register-local --app-id {request.app_id} --workspace {request.workspace_id or "default"} --json
maverick core cli run core.app-sdk.install-local --app-id {request.app_id} --workspace {request.workspace_id or "default"} --json
```
"""
