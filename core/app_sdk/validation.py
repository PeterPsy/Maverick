...
def validate_app_source_completeness(app_root: Path, parsed: ParsedAppContract) -> list[AppSdkValidationIssue]:
    ...
    _validate_frontend_build_contract(app_root, parsed, issues)
    _validate_interface_declarations(parsed, issues)
...
def _validate_frontend_build_contract(app_root: Path, parsed: ParsedAppContract, issues: list[AppSdkValidationIssue]) -> None:
    frontend_root = parsed.contract.entrypoints.frontend
    if not frontend_root:
        return
    package_json = _frontend_package_json(app_root, frontend_root)
    if not package_json.is_file():
        issues.append(
            AppSdkValidationIssue(
                field="package.json",
                message="Frontend apps must include a package.json with a real build script.",
            )
        )
        return
    try:
        ...
        issues.append(
            AppSdkValidationIssue(
                field="package.json",
                message="Frontend apps must include a valid package.json.",
            )
        )
        return
    ...
    if not isinstance(build_script, str) or not build_script.strip():
        issues.append(
            AppSdkValidationIssue(
                ...
                message="Frontend apps must declare a non-empty package build script.",
            )
        )
        return
    normalized = " ".join(build_script.strip().lower().split())
    known_build_tools = ("vite build", "react-scripts build", "next build", "tsc ", "tsc --", "webpack", "rollup")
    if normalized in {"true", "exit 0"} or normalized.startswith("echo ") or (
        "process.exit(0)" in normalized and not any(tool in normalized for tool in known_build_tools)
    ) or (
        "accesssync" in normalized and str(frontend_root).lower() in normalized and not any(tool in normalized for tool in known_build_tools)
    ):
        issues.append(
            AppSdkValidationIssue(
                field="package.json.scripts.build",
                message="Frontend apps must use a real build script, not a no-op artifact existence check.",
            )
        )


def _frontend_package_json(app_root: Path, frontend_mount: str) -> Path:
    frontend_source_root = app_root / frontend_mount.split("/", 1)[0]
    frontend_package = frontend_source_root / "package.json"
    if frontend_package.is_file():
        return frontend_package
    return app_root / "package.json"


def _validate_interface_declarations(parsed: ParsedAppContract, issues: list[AppSdkValidationIssue]) -> None:
    entrypoints = parsed.contract.entrypoints
    declared_surfaces = {
        "backend": bool(entrypoints.backend),
        "cli": bool(entrypoints.cli),
        "mcp": bool(entrypoints.mcp),
        "reference": bool(parsed.contract.capabilities.reference_entities),
        "view": bool(parsed.contract.capabilities.views),
        "widget": bool(parsed.contract.widgets),
    }
    for index, provided in enumerate(parsed.contract.provides):
        missing_surfaces = [surface for surface in provided.surfaces if not declared_surfaces.get(surface, False)]
        if missing_surfaces:
            issues.append(
                AppSdkValidationIssue(
                    field=f"provides[{index}].surfaces",
                    message=(
                        f"Provided interface `{provided.interface}` declares unavailable surface(s): "
                        f"{missing_surfaces}."
                    ),
                )
            )
