"""Official frontend build lifecycle operation for mounted apps."""
...
def build_workspace_app_frontend(
    ...
    *,
    ...
    if not parsed.contract.lifecycle.rebuild:
        raise AppLifecycleError(f"App `{app_id}` does not declare lifecycle.rebuild support.")
...
def app_supports_frontend_build(store: AppStore, *, binding: WorkspaceAppBindingRecord, start_path: Path | None = None) -> bool:
    """Return whether a workspace app binding exposes the official frontend build lifecycle."""
    try:
        source_root, parsed = resolve_workspace_app_surface(store, binding=binding, start_path=start_path)
    except AppLifecycleError:
        return False
    if not parsed.contract.lifecycle.rebuild or parsed.contract.entrypoints.frontend is None:
        return False
    try:
...
def _frontend_build_root(*, source_root: Path, frontend_mount: str) -> Path:
    ...
    if (source_root / "package.json").is_file():
        return source_root
    raise AppLifecycleError("Frontend rebuild requires a package.json with a build script.")
