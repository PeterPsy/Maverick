...
def _effective_workspace_mode(workspace_id: str) -> ExecutionMode:
    return resolve_workspace_execution_profile(
        workspace_id,
        governance=SimpleNamespace(allow_full_access_runtime=workspace_id == "default"),
...
def ensure_app_compatible(*, compatibility: AppCompatibilityDescriptor, workspace_id: str) -> None:
    ...
    workspace_mode = _effective_workspace_mode(workspace_id)
    if supported_modes and not _supports_workspace_mode(supported_modes, workspace_mode):
        raise AppCompatibilityError(
            f"App does not support workspace mode `{workspace_mode}` for workspace `{workspace_id}`."
        )


def _supports_workspace_mode(supported_modes: list[ExecutionMode], workspace_mode: ExecutionMode) -> bool:
    if workspace_mode in supported_modes:
        return True
    return workspace_mode == "full-access" and "sandbox" in supported_modes
