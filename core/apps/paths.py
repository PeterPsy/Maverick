...
def installed_app_root(app_id: str, start_path: Path | None = None) -> Path:
    ...
    return installed_apps_root(start_path=start_path) / validate_app_id(app_id)
...
def workspace_app_source_root(workspace_id: str, app_id: str, start_path: Path | None = None) -> Path:
    """Return one workspace-local app source root with app-id validation."""
    return workspace_apps_root(workspace_id=workspace_id, start_path=start_path) / validate_app_id(app_id)
...
def workspace_app_data_root(workspace_id: str, app_id: str, start_path: Path | None = None) -> Path:
    ...
    return workspace_paths(workspace_id=workspace_id, start_path=start_path).data / validate_app_id(app_id)
