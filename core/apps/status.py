"""Workspace app status transitions and data removal."""
...
def purge_workspace_app_data(*, workspace_id: str, app_id: str, start_path: Path | None = None) -> Path:
    ...
    workspace_data_root = data_root.parent.resolve()
    resolved = data_root.resolve(strict=False)
    if resolved == workspace_data_root or workspace_data_root not in resolved.parents:
        raise AppLifecycleError(f"Refusing to purge app data root outside workspace data: `{data_root}`.")
    if data_root.exists():
