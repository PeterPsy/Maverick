...
def app_sdk_command_specs(
    *,
    ...
    return [
        ...
        (_definition("package", "Package a valid Maverick app source tree."), _package_handler(start_path)),
    ]
...
def _package_handler(start_path: Path | None):
    def _handler(arguments: dict[str, Any], context: CliInvocationContext) -> dict[str, Any]:
        if arguments.get("app_root") or arguments.get("output_path"):
            raise RuntimeError("App SDK package requires `app_id`; app_root/output_path are not part of the agent CLI contract.")
        app_id = str(arguments.get("app_id") or "").strip()
        if not app_id:
            raise RuntimeError("App SDK package requires `app_id`.")
        paths = workspace_paths(workspace_id=_workspace_id(arguments, context), start_path=start_path)
        app_root = paths.apps / app_id
        output_path = paths.generated_storage / f"{app_id}.tar.gz"
        return asdict(package_app_source(app_root, output_path=output_path))
