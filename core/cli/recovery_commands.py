...
from core.cli.core_command_helpers import FULL_ACCESS_WORKSPACE, OPERATOR_ONLY, WORKSPACE_SAFE, core_cli_command
    ...
    *,
    ...
    def _recovery_backend_restart_handler(arguments: dict[str, Any], context: CliInvocationContext) -> dict[str, Any]:
        if context.caller_kind != "full_access_agent":
            raise CliInvocationNotAllowedError("This CLI command is restricted to full-access runtime agents.")
        if context.workspace_id != "default":
            raise CliInvocationNotAllowedError("This CLI command is restricted to the default workspace.")
