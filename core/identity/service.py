...
def update_user(
    ...
    *,
    ...
    if user.is_active and not saved.is_active:
        store.revoke_auth_sessions_for_user(saved.user_id, now=updates["updated_at"])
    return saved
...
def set_user_password(
    ...
    *,
    ...
    saved = store.save_password_credential(credential)
    store.revoke_auth_sessions_for_user(user_id, now=timestamp)
    return saved
    ...
    *,
    ...
    try:
        ...
        if password is not None:
            _ensure_bootstrap_admin_password(identity_store, user=user, password=password, now=timestamp)
...
def _ensure_bootstrap_admin_password(
    identity_store: IdentityStore,
    *,
    user: UserRecord,
    password: str,
    ...
    try:
        ...
        set_user_password(identity_store, user_id=user.user_id, password=password, now=now)
        return
