"""Expiring preview certification for Gemini Interactions and the shared loop."""
...
def ensure_google_preview_certificate(
    store: ProviderStore,
    *,
    definition: AgenticProfileDefinition,
    adapter: object,
) -> CapabilityCertificate:
    """Publish fixture-backed preview evidence for the exact implementation bundle."""
    try:
    ...
    evidence_refs = (
        "platform-evidence:repository:tests/unit/providers/test_google_interactions_codec.py",
        "platform-evidence:repository:tests/unit/providers/test_google_interactions_transport.py",
        "platform-evidence:repository:tests/unit/runtime_state/test_hosted_agentic_loop.py",
        "platform-evidence:documentation:google-agentic-certification-matrix-2026-08-16",
    )
    result_summary_digest = canonical_digest(
        {
            ...
            "contract": (
                "stream-function-call-stateful-stateless-private-state-"
                "sequential-tool-cancel-recover"
            ),
    ...
    certificate = CapabilityCertificate(
        ...
        expires_at=evidence.recorded_at + timedelta(days=GOOGLE_CERTIFICATION_VALIDITY_DAYS),
    )
