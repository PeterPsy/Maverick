...
class BackendWatchdogState:
    ...
    last_blocked_detail: str | None = None
    ...
    def from_dict(cls, payload: dict[str, Any]) -> "BackendWatchdogState":
        return cls(
            ...
            last_blocked_at=_parse_datetime(payload.get("last_blocked_at")),
            last_blocked_reason=payload.get("last_blocked_reason"),
            last_blocked_detail=payload.get("last_blocked_detail"),
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "last_blocked_at": _format_datetime(self.last_blocked_at),
...
def mark_rescue_agent_started(state: BackendWatchdogState, *, now: datetime | None = None) -> BackendWatchdogState:
    ...
    return replace(
        state,
        last_rescue_started_at=now or _now(),
        last_blocked_at=None,
        last_blocked_reason=None,
        last_blocked_detail=None,
    )
...
def mark_rescue_agent_blocked(
    ...
    *,
    ...
    """Record that rescue escalation was blocked before an agent started."""
    return replace(
        state,
        last_blocked_at=now or _now(),
        last_blocked_reason=reason,
        last_blocked_detail=detail,
    )
