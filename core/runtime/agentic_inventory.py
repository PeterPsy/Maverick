...
def inventory_remote_agentic_sessions(
    ...
    for session in store.list_all_sessions():
        ...
        request_count = _distinct_provider_event_count(
            events,
            _PROVIDER_REQUEST_EVENT_TYPES,
        )
        acceptance_count = _distinct_provider_event_count(
            events,
            _PROVIDER_ACCEPTANCE_EVENT_TYPES,
        )
        observed_provider_steps = max(request_count, acceptance_count)
        final_output_count = sum(
            1 for event in events if event.event_type == "runtime.output.final"
        )
        ledger_proposal_count = len(invocations)
        unaccounted = max(
            0,
            observed_provider_steps - ledger_proposal_count,
        )
        pending_count = sum(
        ...
        if unaccounted:
            reason_codes.add("provider_acceptance_without_ledger_proposal")
        if pending_count:
...
def _distinct_provider_event_count(events: list, event_types: frozenset[str]) -> int:
    identities: set[str] = set()
    for event in events:
        if event.event_type not in event_types:
            continue
        request_id = str(event.payload.get("request_id") or "").strip()
        identities.add(request_id or f"event:{event.event_id}")
