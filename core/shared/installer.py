...
from core.shared.browser_origin_tls import (
    EXTERNAL_WILDCARD_TLS_MODE,
    MANAGED_EXACT_TLS_MODE,
    SUPPORTED_TLS_MODES,
)
...
from core.shared.installer_tls import (
    ...
    managed_browser_origin_tls_errors,
...
class InstallerConfig:
    ...
    browser_origin_tls_mode: str = EXTERNAL_WILDCARD_TLS_MODE
    browser_origin_tls_root: str = ""
    browser_origin_acme_webroot: str = ""
    browser_origin_acme_ca: str = ""
    browser_origin_tls_nginx_group: str = "www-data"
    ...
    @property
    def managed_browser_origin_tls_root(self) -> str:
        return self.browser_origin_tls_root or "/var/lib/maverick/browser-origin-tls"

    @property
    def managed_browser_origin_acme_webroot(self) -> str:
        return self.browser_origin_acme_webroot or "/var/lib/maverick/browser-origin-acme"
...
def render_nginx_config(config: InstallerConfig, *, force_https: bool = False) -> str:
    ...
    if config.hosted_sidecars:
        if config.browser_origin_tls_mode == MANAGED_EXACT_TLS_MODE:
            template_name = (
...
def render_install_env_file(config: InstallerConfig) -> str:
    ...
    if config.hosted_sidecars:
        ...
        values["MAVERICK_BROWSER_ORIGIN_TLS_MODE"] = config.browser_origin_tls_mode
        if config.browser_origin_tls_mode == MANAGED_EXACT_TLS_MODE:
            values["MAVERICK_BROWSER_ORIGIN_TLS_ROOT"] = config.managed_browser_origin_tls_root
            values["MAVERICK_BROWSER_ORIGIN_ACME_WEBROOT"] = config.managed_browser_origin_acme_webroot
            if config.browser_origin_acme_ca:
                values["MAVERICK_BROWSER_ORIGIN_ACME_CA"] = config.browser_origin_acme_ca
    lines = [
...
def build_install_manifest(config: InstallerConfig) -> dict[str, Any]:
    ...
    return {
        ...
        "browser_origin_tls_mode": None if not config.hosted_sidecars else config.browser_origin_tls_mode,
        "browser_origin_tls_root": (
            config.managed_browser_origin_tls_root
            if config.hosted_sidecars and config.browser_origin_tls_mode == MANAGED_EXACT_TLS_MODE
            else None
        ),
    }
...
def preflight_check(
    ...
    *,
    ...
    if config.hosted_sidecars:
        ...
        elif config.browser_origin_tls_mode not in SUPPORTED_TLS_MODES:
            errors.append("browser-origin TLS mode must be `external_wildcard` or `managed_exact`")
        elif live_apply and config.browser_origin_tls_mode == EXTERNAL_WILDCARD_TLS_MODE:
            errors.extend(
                hosted_sidecar_tls_errors(
        ...
        elif config.browser_origin_tls_mode == MANAGED_EXACT_TLS_MODE and (
            not Path(config.managed_browser_origin_tls_root).is_absolute()
            or not Path(config.managed_browser_origin_acme_webroot).is_absolute()
        ):
            errors.append("managed browser-origin TLS paths must be absolute")
        elif live_apply and not request_tls:
            errors.extend(
                managed_browser_origin_tls_errors(
                    hostname=str(config.hostname),
                    tls_root_value=config.managed_browser_origin_tls_root,
                )
            )
        if (
            live_apply
            and request_tls
            and config.browser_origin_tls_mode == MANAGED_EXACT_TLS_MODE
            and shutil.which("runuser") is None
        ):
            errors.append("runuser not found in PATH")
    if config.build_frontends:
...
def request_tls_certificate(config: InstallerConfig, *, run_command: Any = None) -> None:
    """Request or renew platform TLS and any installer-managed origin probes."""
    ...
    if config.hosted_sidecars and config.browser_origin_tls_mode == MANAGED_EXACT_TLS_MODE:
        prepare_managed_browser_origin_tls(config, run_command=runner)
        runner(managed_browser_origin_probe_command(config))
    if config.live_nginx_conf_path is not None:
...
def prepare_managed_browser_origin_tls(config: InstallerConfig, *, run_command: Any = None) -> None:
    """Create least-privilege private, published, and HTTP-01 directories."""
    if not config.hosted_sidecars or config.browser_origin_tls_mode != MANAGED_EXACT_TLS_MODE:
        return
    runner = run_command or run_privileged_command
    tls_root = Path(config.managed_browser_origin_tls_root)
    private_root = tls_root / "private"
    served_root = tls_root / "served"
    webroot = Path(config.managed_browser_origin_acme_webroot)
    for path, owner, group, mode in (
        (tls_root, config.service_user, config.service_group, "0711"),
        (private_root, config.service_user, config.service_group, "0700"),
        (served_root, config.service_user, config.browser_origin_tls_nginx_group, "2750"),
        (served_root / "hosts", config.service_user, config.browser_origin_tls_nginx_group, "2750"),
        (served_root / "releases", config.service_user, config.browser_origin_tls_nginx_group, "2750"),
        (webroot, config.service_user, config.browser_origin_tls_nginx_group, "0755"),
        (
            webroot / ".well-known" / "acme-challenge",
            config.service_user,
            config.browser_origin_tls_nginx_group,
            "0755",
        ),
    ):
        runner(["install", "-d", "-o", owner, "-g", group, "-m", mode, str(path)])


def managed_browser_origin_probe_command(config: InstallerConfig) -> list[str]:
    """Return the service-user command that obtains both routed probe names."""
    if not config.hostname:
        raise ValueError("Managed browser-origin TLS requires a hostname.")
    python = config.repository_root / ".venv" / "bin" / "python"
    executable = str(python if python.is_file() else Path("python3"))
    hosts = [
        sidecar_tls_probe_url(config.hostname).removeprefix("https://").removesuffix("/"),
        app_frame_tls_probe_url(config.hostname).removeprefix("https://").removesuffix("/"),
    ]
    command = [
        "runuser",
        "--user",
        ...
        executable,
        "-m",
        "core.shared.browser_origin_tls",
        "--installation-domain",
        config.hostname,
        "--tls-root",
        config.managed_browser_origin_tls_root,
        "--acme-webroot",
        config.managed_browser_origin_acme_webroot,
        "--group-key",
        f"browser-origin-probes:{config.hostname}",
    ]
    if config.browser_origin_acme_ca:
        command.extend(("--acme-ca", config.browser_origin_acme_ca))
    for host in hosts:
...
def _template_substitutions(config: InstallerConfig) -> dict[str, str]:
    return {
        ...
        "{{HOSTNAME_REGEX}}": re.escape(config.hostname or ""),
        "{{BROWSER_ORIGIN_TLS_ROOT}}": config.managed_browser_origin_tls_root,
        "{{BROWSER_ORIGIN_ACME_WEBROOT}}": config.managed_browser_origin_acme_webroot,
        "{{BROWSER_ORIGIN_DEFAULT_HOST}}": app_frame_tls_probe_url(config.hostname or "")
        .removeprefix("https://")
        .removesuffix("/"),
