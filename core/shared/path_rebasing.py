...
def rebase_local_state_paths(*, start_path: Path | None = None) -> list[Path]:
    """Rebase all known local JSON path fields to the current repository root."""
    repository_root = discover_repository_root(start_path=start_path)
    changed_paths: list[Path] = []

    control_store_roots = (
        repository_root / "data" / "control-plane" / "json",
        repository_root / ".maverick" / "local-state",
    )
    targets = []
    for control_store_root in control_store_roots:
        targets.extend(
            [
                (control_store_root / "apps" / "app_sources.json", ("source_path",)),
                (
                    control_store_root / "apps" / "workspace_local_app_projects.json",
                    ("project_root",),
                ),
                (control_store_root / "apps" / "workspace_app_bindings.json", ("data_root",)),
            ]
        )
