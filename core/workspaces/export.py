...
DEFAULT_EXPORT_EXCLUDED_SUFFIXES = {".pem", ".key", ".sqlite-wal", ".sqlite-shm"}
...
def include_in_workspace_export(*, file_path: Path, workspace_root: Path) -> bool:
    ...
    if any(segment in DEFAULT_EXPORT_EXCLUDED_SEGMENTS or segment in DEFAULT_EXPORT_EXCLUDED_NAMES for segment in parts):
        return False
    if file_path.name in DEFAULT_EXPORT_EXCLUDED_NAMES or file_path.suffix in DEFAULT_EXPORT_EXCLUDED_SUFFIXES:
        return False
