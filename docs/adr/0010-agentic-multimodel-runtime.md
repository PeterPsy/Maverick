...
contained path.
OpenRouter GLM profile 6 instead uses `remote-agentic-full-workspace@1`: an
...
accepted as complete. The current definitions are Google revision 70 and
OpenRouter GLM revision 6. They use `maverick-hosted-tool-loop==59`; the GLM
profile is bound to
`openrouter-relace-glm-5-3-flash@2`. Historical profiles for other
targets grant no authority. Their suite-67 certification manifests retain distinct
