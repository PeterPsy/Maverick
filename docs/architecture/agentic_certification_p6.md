...
and Antigravity remain **NO-GO**. Until suite 64 is published and its binding is
...
That pass is historical. The current successor uses suite 64 / TCB 54 / hosted
adapter 57 / Google recipe 26 and profile 68 / OpenRouter GLM recipe 29 and
profile 3,
...
Candidate 53 narrows that bounded Google diagnostic after the suite-52 live
observation identified `interaction.created` as the rejected event. Without
copying the event, it distinguishes duplicate lifecycle events, missing or
invalid partial payload/id, exact-model resource-name or revision forms, and an
otherwise mismatched model. Decoder acceptance and the public error remain
unchanged. The refinement advances hosted adapter 46, suite 53, shared TCB 43,
and API profiles 57/56. Hosted recipe 25, Google codec 4, Antigravity
adapter/profile 4 and recipe 3, the native artifact, and Codex revision 15 are
unchanged.

Candidate 54 narrows only the content-free ID diagnostic after the suite-53
live observation reached `interaction.created` with an invalid ID shape. It
distinguishes absent, null, non-string, empty, and oversized IDs without
retaining the value or changing decoder acceptance. The refinement advances
hosted adapter 47, suite 54, shared TCB 44, and API profiles 58/57. Hosted
recipe 25, Google codec 4, Antigravity adapter/profile 4 and recipe 3, the
native artifact, and Codex revision 15 are unchanged.

Candidate 55 accepts Google's observed empty lifecycle ID only for the
certified stateless `store=false` path. That ID is not exposed as a provider
response identity and is never persisted for continuation; stateful execution,
non-string/oversized IDs, mismatched completion IDs, model drift, and every
other invalid event still fail closed. This codec change advances Google codec
5, hosted adapter 48, suite 55, shared TCB 45, and API profiles 59/58. Hosted
...
1. **Candidate identities and deterministic corpus:** hosted adapter 57,
   Google recipe 26/profile 68, OpenRouter GLM recipe 29/profile 3, suite 64,
   canonical TCB manifest 54, Antigravity adapter 5 / recipe 4, and active Codex revision
   15.
