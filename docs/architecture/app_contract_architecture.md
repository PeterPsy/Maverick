...
Before reserving an app runtime stream or publishing a new agentic app session,
Core resolves the enabled workspace model configuration permitted for the
authenticated actor, runs remote-agentic admission, and creates the direct
execution binding itself. An app may request a stable
`workspace_profile_binding_id` and reasoning effort, but it cannot classify
session data or supply provider authority, credentials, an egress attestation,
or an execution-binding snapshot. Core verifies the enabled selection and real
permission boundaries at admission; no profile revision, rollout, recipe digest,
or certificate metadata participates.
...
interactive browsing, workspace reads, writes, shell, or delegation. This is
