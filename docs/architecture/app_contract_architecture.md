...
The core invokes this hook only for runtime sessions whose `source_app_id` matches the enabled workspace app binding. The hook receives terminal runtime events such as `runtime.turn.completed` and `runtime.turn.failed`, plus the `runtime_session_id`, `turn_id`, terminal status, final output text when available, and failure reason when available. Cancellation callbacks also receive a stable `runtime_event_id`: delivery is at least once, and apps must use that id as the idempotency key if a process failure causes callback replay. Direct runtime turn submissions against a source-app session and backend restart recovery both dispatch a non-terminal `runtime.turn.queued` event when the core queues a new turn, so the source app can keep its app-owned projection pointing at the active turn before a later terminal event arrives.
