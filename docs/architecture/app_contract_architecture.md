...
Apps may also return `runtime_turn_interrupt_requests` for turns that belong to a user-visible runtime session sourced by that same app. The core validates workspace, visibility, and source-app ownership, performs the generic interrupt operation, records the runtime terminal event, and dispatches the same source-app runtime event hook. The interrupt result is successful when that request created the durable cancellation intent or claimed the terminal outbox; a turn worker may drain the outbox concurrently without changing the successful result of the intent owner. The app still owns any product-level decision to mark its own node, job, or workflow as stopped or failed.
