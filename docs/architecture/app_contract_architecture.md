...
Each request may include `agent_id`, optional `runtime_session_id`, `system_prompt` or a generic dependency-backed `system_prompt_request`, `skill_ids`, `input_text`, `app_references`, and a callback action. The platform stamps the created session with the requesting app as `source_app_id`; when the request comes from an authenticated backend, CLI, or MCP invocation, the human caller is also stamped as `owner_user_id` and `created_by_user_id` so ordinary owner-scoped runtime actions such as stopping a chat turn continue to work. The core then submits the turn through the runtime and invokes the app callback with the created `runtime_session_id` and `turn_id` or an error. The callback lets the app persist its own projection state without the core writing app-owned data.
