...
Before publishing a new agentic app session, Core resolves the workspace profile permitted for the authenticated actor and mints the immutable execution binding itself. An app may request a stable `workspace_profile_binding_id`, reasoning effort, and governed remote-data declaration, but it cannot supply provider authority, a capability certificate, or an execution-binding snapshot.
