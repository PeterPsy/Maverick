...
has its legacy terminal event is atomically adopted as fully delivered, so the
first deployment of the outbox format does not replay an application callback.
