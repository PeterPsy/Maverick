...
allowlist, rechecks the executable against an exact reviewed runtime version
immediately before each Research launch, verifies the private home reported at
initialization, and applies a positive allowlist to raw app-server item types
before parsing them. Only reviewed passive items and native web search may
continue; operational or unknown items terminate the process. Core rejects
unknown or unattested native transports. Thus new
...
`WS /ws/runtime/sessions/<session_id>` sends a `runtime.snapshot` frame containing session metadata, a bounded recent tail of persisted events, and the runtime turn records needed to project that bounded page even when it starts mid-turn, then live `runtime.event` frames from the runtime event bus. Clients request persisted pages over the same connection with `runtime.history.before`, `runtime.history.after`, `runtime.history.latest`, or `runtime.history.around` and receive `runtime.history.page`. Pages carry the relevant turn records, direction, both availability flags and an optional client `request_id` (1–128 characters) for discarding superseded responses. Before/after cursors are exclusive and scoped to the authenticated session; browsing never advances the live replay cursor. Forward reads visit session archive chunks without materializing the complete archive. Latest replaces a client history window after an explicit return to live. Around restores a reading position using a persisted `around_event_id`, includes that event exactly once, and reads bounded pages on both sides under the same authenticated session scope. An absent anchor returns an empty page; event lookup never falls through into another session. Initial chat load must remain bounded and independent of total transcript length. `WS /ws/runtime/threads` sends a `runtime.thread.snapshot` frame containing only user-visible workspace runtime threads, ordered by the most recent accepted user message for each thread, then live `runtime.thread.changed` frames from the runtime thread event bus. Thread frames include core-derived response completion metadata such as `last_completed_response_at`, `last_completed_turn_id`, and the viewer-specific `has_unread_completed_response` boolean. They may also expose bounded, non-sensitive session classifiers needed by thread catalogs, currently `runtime_profile` and `device_use_enabled`; raw Device Use bindings and bearer tickets remain private. Per-user read receipt storage remains internal to the core runtime thread record and must not be exposed as a raw user-id map in app payloads.
...
Canonical samples, stream cursors, session summaries, hourly/daily provider-model buckets, and redaction-safe quota observations belong to the independent `core.usage` persistence owner. `MAVERICK_USAGE_STORE=sqlite` selects `data/control-plane/usage/usage.sqlite`, requires a verified WAL-safe SQLite runtime (at least 3.51.3), and fails closed if explicit migration has not supplied a compatible database. Each observation atomically commits its identity, normalized sample, stream cursor and projections with `synchronous=FULL`; no observation is buffered for later durability. A late cumulative sample adjusts only its indexed predecessor/successor neighborhood, including reset and model boundaries. Summary and chart GETs read projections without repairing or writing history. Runtime cleanup removes a session's samples and contributions transactionally.

The `document` Usage adapter remains an explicit transition exception: before cutover it uses the current control-plane JSON/Mongo collections, and after a verified reverse export it supports rollback. Operator-only Usage migration surfaces implement prepare, validate, cutover, backup, repair and rollback under an interprocess fence. Explicit repair backs up the current database and atomically rebuilds only derived tables from committed observations, validating their counters and stream cursors before commit; samples, retained numeric provider observations and quotas remain unchanged. They retain an immutable original export, verify canonical identities and projections, and reject source changes after preparation. Cutover is performed with producers drained and completed by restarting with the requested environment setting; stale adapter instances cannot keep writing. Rollback exports the current SQLite state, including post-cutover observations, before retiring the database. Ordinary startup neither creates a missing SQLite database nor migrates it. General control-plane migrations exclude Usage collections when the SQLite owner is configured and preserve the old Usage source archive during cleanup. Owner-aware status reports Usage separately; backups use SQLite's backup API to include committed WAL pages. Historical coverage starts when metering is deployed; neither the API nor the UI may imply that older unobserved turns were reconstructed.
...
For an agentic session, final-output and terminal-event `provider_id` is the model provider pinned by the immutable execution binding. A shared hosted runtime engine is routing infrastructure, not the owner of the provider outbox delivery. HTTP and background turn finalization must reuse the pinned model-provider identity when reconciling a stable `runtime.output.final`; it must not relabel that event with the runtime-engine id or turn a successfully committed final output into an identity conflict.
