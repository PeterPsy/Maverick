...
Maverick-managed Codex runtime config must also own provider-hook configuration. The adapter installs a runtime-local `maverick_codex_post_tool_use_hook.py` script under the session `bin/`, removes inherited operator-home `hooks.*` sections, enables the Codex `hooks` feature, and registers a managed `[[hooks.PostToolUse]]` entry for `Bash`. The hook calls Maverick's runtime-token provider-hook API and, for large or redacted shell output, returns `decision: "block"`, `continue: false`, and a compact replacement reason so Codex continues from the compacted provider-history message instead of the original shell result. This is a Codex `Bash` hook guarantee, not a blanket claim for all provider tools.
