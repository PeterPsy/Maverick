...
Backend host startup is also the place for complete app/provider bootstrap. Sidecar CLI and MCP wrapper processes that only serve read-only discovery, inspect, SDK docs/templates, or developer-context commands should load the control-plane stores without reinstalling built-in apps, rerunning app hooks, registering provider definitions from heavy adapters, or bootstrapping admin credentials. Mutating sidecar commands may request full bootstrap when they need host-side app/provider state creation.
...
For local sidecar invocations, read-only wrapper commands such as `apps list`, CLI/MCP `list` and `inspect`, SDK docs/templates, and developer-context reads must not perform the backend host bootstrap path. They should use the existing persisted control-plane state. Runtime agents should normally reach these commands through the runtime HTTP shim rather than through the developer wrapper.
