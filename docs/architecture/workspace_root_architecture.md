...
Provider adapters may also bind specific development helper binaries into this directory when those tools are required for ordinary workspace work. These must be explicit file-level read-only binds, not broad package-root, home-directory, or repository mounts. For Codex, the adapter binds the packaged standalone `rg` binary into `runtime/sessions/<runtime_session_id>/bin/rg` so agents can search inside their workspace without being able to read the operator's Codex package tree or any non-workspace data. The adapter may also install a minimal static shell as `runtime/sessions/<runtime_session_id>/bin/sh` and bind that file to `/bin/sh` inside the workspace sandbox so provider command execution can run shell commands without mounting host `/bin`, `/usr`, `/lib`, or `/lib64`.

Inside a non-default workspace sandbox, the workspace must be mounted at the synthetic path `/workspace`. Host coordinates such as `/home/ubuntu/maverick/workspaces/<workspace_id>` must not be visible to the agent. Runtime-local paths such as `CODEX_HOME`, `TMPDIR`, and `MAVERICK_RUNTIME_ROOT` must also be expressed below `/workspace/runtime/...` when the runtime root lives inside the workspace.
