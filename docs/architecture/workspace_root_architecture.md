...
Provider adapters must make that runtime `bin/` directory the first `PATH` entry for the provider process and for provider-managed shell tool environments. If the provider has its own shell environment policy, as Codex does, the runtime home configuration must explicitly set `PATH` so `command -v maverick` resolves to the session-local shim. A developer/operator wrapper such as `scripts/maverick` may exist outside the workspace root, but when it is accidentally invoked inside a runtime session with a runtime token it must delegate to the session shim instead of bootstrapping the platform directly.
