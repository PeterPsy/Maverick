...
Provider adapters may place managed provider hook scripts in the same runtime `bin/` directory when the hook is session-scoped and uses only runtime-issued authority. For Codex, Maverick installs a `PostToolUse` hook script there and registers it in the session `CODEX_HOME/config.toml` so large `Bash` results can be redacted and compacted before the provider continues from the tool output.
