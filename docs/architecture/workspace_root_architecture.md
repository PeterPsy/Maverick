...
Control-plane persistence for workspace registry, membership, governance, quota, and active workspace selection may target JSON, MongoDB, or a future adapter, but their semantics must not depend on adapter-specific documents or driver APIs.
...
- database-specific queries and update operators stay inside store adapters only
...
- `/api/workspaces` may list active workspace records the user is an active member of
- `/api/workspaces/active` may switch the active workspace for that user session only when the target workspace and membership are active
...
- which record-centric records exist in this workspace
...
Workspace-scoped runtime thread records live under:

```text
/workspaces/<workspace_id>/runtime/threads.json
```
...
Provider adapters must make that runtime `bin/` directory the first `PATH` entry for the provider process and for provider-managed shell tool environments. If the provider has its own shell environment policy, as Codex does, the runtime home configuration must explicitly set `PATH` so `command -v maverick` resolves to the session-local shim. A provider adapter must not launch runtime sessions through a host-level provider wrapper that rewrites `PATH`, clears the runtime home, or otherwise strips the managed runtime environment; when such a wrapper is discovered, the adapter must resolve the real provider entrypoint or packaged runtime binary for session launch while leaving the operator wrapper untouched for non-runtime use. A developer/operator wrapper such as `scripts/maverick` may exist outside the workspace root, but when it is accidentally invoked inside a runtime session with a runtime token it must delegate to the session shim instead of bootstrapping the platform directly.

Provider adapters may place provider hook scripts in the same runtime `bin/` directory when the hook is session-scoped and uses only runtime-issued authority. For Codex, Maverick installs a `PostToolUse` hook script there, disables Codex `unified_exec` for Maverick-managed sessions, registers the hook in the session `CODEX_HOME/config.toml`, and writes the matching Codex `hooks.state` trusted hash so large `Bash` results can be redacted and compacted before the provider continues from the tool output when Codex accepts and runs that hook under its hook-trust policy.
...
The shim must support the same discovery-first command families documented for agents, including `maverick apps list --json`, `maverick core cli list --json`, `maverick app <app_id> cli list --json`, and `maverick app <app_id> frontend build --json`. Runtime shims forward those commands to a core-owned runtime CLI API, which resolves the workspace and runtime session from a signed runtime token carrying workspace, session, execution-mode, token id, and expiry claims. That token id must also have an active store-backed lifecycle record, so the platform can revoke runtime API tokens and reject unregistered, expired, mismatched, or revoked tokens before enforcing the command's invocation policy. Hosted full-access tool processes additionally bind the token to one active `runtime_turn_id`; terminalization revokes it even when the child process survives. Native provider launch tokens remain session-scoped because their process spans turns. The API accepts `output_profile: "full"` or `output_profile: "provider_compact"` in the runtime-token request body; `full` is the API default for compatibility, while the shim requests `provider_compact` by default so large textual CLI response fields are compacted before provider consumption. Provider compaction must preserve explicit document-body fields for developer-context and Storage read surfaces from truncation when shortening would corrupt the requested source material; redaction still applies. Runtime shims must not impose a short fixed client timeout on forwarded CLI or SDK calls because app-owned operations such as renders and exports may legitimately run longer than ordinary request/response commands. Frontend rebuild runs host-side build tooling and is denied to sandbox runtime sessions; agents must use an allowed full-access or operator path for that operation.
...
In workspace paths, `<app_id>` means the workspace-local app id for that binding. It may match the app artifact's public catalog id, but it is not required to match. The platform app binding must preserve both the `public_app_id` from the app contract and the `local_app_id`/mount id used inside this workspace.
...
- workspace-owned app outputs that are not just storage files
...
/workspaces/acme/data/records/indices/
...
- apps do not treat another app id as an internal dependency
...
- cross-app workflows use declared app interfaces and official hosted surfaces
...
- installed app capabilities do not call each other through private filesystem or database access
- installed app capabilities may compose through contract-declared interfaces selected per workspace
...
- cross-app consumers declare interface requirements by type
- the shell coordinates provider selection for the workspace
- agents or cross-app apps decide when to use selected provider surfaces
...
- backend env-building, provider-specific launch configuration, turn execution, turn interruption, and provider runtime cleanup belong in the provider adapter
...
The current implementation resolves provider selection in the core and currently ships only one concrete runtime backend adapter:
...
A workspace may also start with no configured or available AI provider.

That is a valid post-install state. It means runtime turns and recovery actions that require a provider are blocked until an authorized setup flow selects a provider; it does not mean the workspace, core host, or installed apps failed to initialize.

Provider selection is workspace governance state controlled through core provider APIs, not app-owned data and not browser-local preference. Only a workspace admin, platform admin, or later explicit governance capability may change the workspace-wide provider/model selection. Ordinary workspace members may read provider status but must not change the setting for everyone. Once selected, the configured provider is the authoritative backend for new runtime sessions in that workspace. The same configured provider is also the provider choice used by automatic backend rescue when the platform recovery service needs an AI runtime.

If no provider is configured or available, provider and runtime status surfaces should report that directly, for example with `active_provider: null` and a stable `blocked_reason`. They must not silently choose Codex or the first registry candidate.

The Codex backend must be conversation-continuous.

For interactive chat and agent sessions, Codex is driven through the Codex app-server protocol:

- the core runtime creates one Maverick runtime session
- the Codex provider adapter launches `codex app-server --listen stdio://`
- the adapter creates or resumes one Codex provider thread for that runtime session
- every user turn is sent with `turn/start` to that same provider thread
- provider app-server events are normalized into Maverick runtime events and streamed over the core runtime WebSocket

If the provider app-server subprocess exits before emitting a terminal turn event, the runtime must treat that turn as terminally failed or cancelled according to the canonical turn record and must release the session execution lock so queued follow-up turns can start.

Core runtime stores chat thread metadata and the linked Maverick `runtime_session_id`. The chat app may render and update that core-owned record through runtime APIs, but it must not persist its own thread catalog, own the Codex provider thread id, or reconstruct provider memory itself.

The provider thread id is core runtime/provider state.

This keeps the app boundary clean while preserving real conversation memory.

If the browser refreshes or the user leaves and reopens the chat, the UI reloads transcript from Maverick runtime events and the next user turn continues through the same provider conversation when the backend supports thread resume.
...
Apps that store shared JSON state under this root must use the SDK transactional update helper for read-modify-write operations. Separate locked reads and locked writes are not enough: concurrent app backend, CLI, MCP, or hook invocations can otherwise read the same old state and overwrite each other's changes.
...
In the first implementation, a small app-owned metadata file under `data/<app_id>/` is an acceptable default place to persist the current data schema version and installed app version.
...
For the local hosted bootstrap, identity and workspace control-plane records are stored in the configured control-plane adapter. The default adapter is JSON under `data/control-plane/json`, outside `.maverick`. MongoDB and future adapters are selectable installation choices.

This control-plane persistence is not workspace data:

- user records and password credentials stay in control-plane identity state
- auth sessions stay in control-plane identity state
- workspace registry, membership, governance, quota, and active workspace selection stay in control-plane workspace state
- app-owned operational content remains under `workspaces/<workspace_id>/data/<app_id>/`

Store adapters must not leak database-driver or JSON-file shapes into domain models or app contracts.
...
The local bootstrap host persists workspace-scoped runtime collections under the owning workspace root. Runtime sessions, runtime threads, turns, events, processes, and state must survive host restarts without using installation-local `.maverick/local-state/runtime/` as their storage home.
...
These should automatically appear in the workspace file inventory and storage as uploaded files.

The current core HTTP upload surface is:

```text
POST /api/workspace-files/uploads
```

It writes file bytes under:

```text
/workspaces/<workspace_id>/storage/uploaded/<file_id>/<safe_filename>
```

The response returns stable metadata such as `file_id`, `relative_path`, `sha256`, size, content type, and original filename.
The upload API must reject decoded payloads over the configured platform limit or the workspace's `max_storage_bytes` quota before writing bytes to the workspace filesystem.

Apps such as chat may pass this metadata to runtime turns as attachment references, but they must not inline file bytes in runtime events or store generic uploaded files under their own `data/<app_id>/` directory.

### `storage/generated/`

Files generated by agents or workspace-local tools.

These should automatically appear in the workspace storage as generated assets without requiring a separate persistence ritual.
...
- generated documents
- generated media
- binary assets intended for storage or file inventory
...
- app-specific persisted content that is not primarily a storage file
...
- persistent chat-thread history, which is core runtime state
...
In the `default` workspace, agents run with `full-access` by default when platform and workspace governance policy allow it, and may operate on the server outside the workspace root.

Full-access is execution mode, not administrative authority.

Operations that affect the platform host, such as manually restarting the backend service, must still require explicit platform or workspace admin capability grants. A full-access runtime in `default` may have the operating-system reach needed to perform the action, but the core CLI/MCP/API surface must deny it unless governance grants the corresponding capability.

The `default` workspace may still explicitly request sandboxed execution for a specific runtime session, but that is an override rather than the default behavior.
...
- mounted app backend, CLI, and MCP entrypoints receive secret values only as ephemeral `app_secrets` input from active, non-expired grants that allow `app.backend`, match the delivery target such as `maverick://app.backend/backend`, `maverick://app.backend/cli/<command>`, or `maverick://app.backend/mcp/<tool>`, and match contract-declared logical names; CLI and MCP delivery is further limited to logical names explicitly listed in the command or tool descriptor's `required_secrets`; required delivery fails closed and emits audit/event records if a logical name has no current compatible grant
...
## Storage and File Inventory Model
...
The storage and file inventory should derive from the workspace storage tree, not from a separate persistence ceremony.
...
The Storage app's current derived index is an app-owned JSON inventory at:

```text
/workspaces/<workspace_id>/data/storage/files.json
```

That inventory records stable file ids, current paths, content type, preview kind, size, modified time, and content hash when the bytes entered through a Storage-managed write path. Storage-managed upload, write, Markdown update, folder create/delete, rename, move, and delete actions update the inventory synchronously while holding the workspace Storage write lock. Catalog and reference surfaces may read that index for server-side pagination, folder-scoped filtering, targeted lookup by stable file ids or workspace-relative paths, search, and resolution, but byte-serving and mutations must still validate the current filesystem path before touching the file. Catalog folder scoping must distinguish an omitted folder from an explicit empty folder path, because `folder_path: ""` means the direct children of the selected storage root.

Large Storage transfers must avoid app-backend JSON bodies as the transport. Inline write and read operations stay bounded for CLI, MCP, and preview workflows, while app-owned chunked upload sessions and authenticated media/file-response routes handle large user-facing upload and download workflows. This keeps workspace files under `storage/uploaded/` and `storage/generated/` as the source of truth without requiring the core app backend bridge to accept hundreds of megabytes of JSON.

Out-of-band files under `storage/uploaded/` or `storage/generated/` remain valid workspace artifacts. Storage catalog reconciliation may scan the filesystem to add those files to `files.json`; once indexed, references should use the assigned stable `file_id`. Reconciliation must ignore Storage atomic-write temporary files with the internal `.maverick-storage-write-` prefix and tolerate paths disappearing while a concurrent external writer changes the tree.
...
- the storage sees it as a generated asset
- preview/index metadata may be created asynchronously
- no extra manual persistence dance is required

The Storage app may expose file operations over this derived inventory, but those operations must still resolve against the current workspace storage root before touching bytes. Preview, download, rename, and delete actions must accept either the derived role plus relative path or a normalized `workspace_relative_path`, reject traversal outside `storage/uploaded/` and `storage/generated/`, and treat the filesystem as the final existence check.

Storage cards should show the best available inline preview without requiring a modal open. Browser-previewable media and documents such as images, video, audio, PDFs, Markdown, and text can render directly from Storage's validated read surface. Preview extraction must be bounded before reading whole files into memory. Modern Office formats such as DOCX, PPTX, and XLSX may expose extracted text previews through a Storage-owned preview action with file, archive-entry, and decompressed-byte budgets. Legacy binary formats such as DOC, PPT, and XLS remain represented by typed file previews unless a dedicated conversion/indexing capability is introduced.
...
7. Storage, file APIs, and retrieval must never return artifacts from another workspace.
...
- storage visibility depends on backend records more than on actual file placement
...
5. Make storage and file inventory read from workspace storage as the source of truth.
...
- storage derives automatically from `storage/uploaded/` and `storage/generated/`
