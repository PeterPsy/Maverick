...
The current implementation exposes this through `core/api/runtime_api.py`, with turn execution delegated to `core/runtime/turn_submission.py` so HTTP routing stays separate from runtime orchestration.
...
- WebSocket: `WS /ws/runtime/sessions/{session_id}`

HTTP event replay remains available:

- `GET /api/runtime/sessions/{session_id}/events`
- `GET /api/runtime/turns/{turn_id}/events`

Chat should use WebSocket during active runtime sessions.

Polling `GET /api/runtime/sessions/{session_id}/events` is only a fallback for reconnect, unsupported clients, diagnostics, or initial transcript recovery.

The event domain model must stay the same whether events are delivered by WebSocket or HTTP replay.

The WebSocket stream must not expose chat-specific frames. It carries generic runtime events plus transport-level heartbeat/ack/replay control frames.

Current Codex provider execution emits progress through `codex exec --json`.

The core normalizes provider JSONL into app-agnostic runtime events, including step updates, tool-call lifecycle events, output deltas, final output, and turn terminal events.
...
- authenticate and authorize access to the runtime session at connection time
- reject cross-workspace session access
- accept an optional last seen event id for replay after reconnect
- send ordered runtime events as they are persisted
- send transport heartbeats without storing them as runtime events
- keep final answer, tool calls, step updates, failure, cancellation, and terminal turn events visible in realtime
- allow chat to fall back to HTTP replay without changing transcript projection logic
...
- Carry `client_message_id` and attachment metadata through the queued-turn event payload for generic optimistic UI reconciliation.
...
- `POST /api/workspace-files/uploads` stores uploaded files under the active workspace.
- The returned metadata includes `file_id`, `relative_path`, `sha256`, size, content type, and filename.
- Chat uploads local composer files through that endpoint before submitting the runtime turn.
- Runtime turn payloads carry attachment metadata only; runtime execution and transcript projection do not embed file bytes.
...
- add a small registry client abstraction
- render generic fallback when no enabled widget matches
- embed matching widgets through the core widget registry and iframe frontend mount
- pass only explicit signed widget context generated by the core
- add tests for fallback/projection and registry-driven widget discovery
...
- Use the v3 core WebSocket runtime stream for active runtime event delivery.
...
- [x] Route externally created runtime sessions to Chat through `runtime_session_id` navigation params, with Chat creating or reusing the owning thread record.
- [x] Persist scalar agent metadata on Chat-owned thread records when a runtime session is opened from the Agents app.
- [x] Refresh the chat-owned sidebar widget through generic shell-mediated data invalidation when Chat creates or reuses a thread.
...
- [x] Wire attachment metadata into the runtime submit payload only through the official v3 attachment surface.
- [x] Upload composer attachments through generic core workspace upload storage before runtime submit.
- [x] Render sent human-message attachments once the message is confirmed.
...
- [x] Separate bootstrap/send state from active runtime-turn state; streaming remains deferred until the transport emits deltas.
...
- [x] Render pending human messages, confirmed human messages, and failed-send human messages distinctly.
- [x] Render assistant/provider streaming text without layout jumps.
- [x] Render empty assistant/provider output without producing broken blank bubbles.
- [x] Add heavy-message collapse/expand for very long outputs.
...
- [x] Add copy affordances for assistant text.
...
- [x] Show last activity timestamp only if derived from runtime events or session state.
...
- [x] Define the runtime-event-to-transcript projection for tool-call started, updated, completed, and failed states.
- [x] Render inline tool-call cards when runtime events expose tool-call data.
- [x] Add a detail panel or expandable row for tool-call payload details if the visual source includes it.
- [x] Keep tool-call rendering passive until the corresponding core runtime events are stable.
- [x] Port generic structured message fallback rendering.
- [x] Do not reintroduce compile-time cross-app widget imports.
- [x] Embed concrete widgets only through the registry-driven widget host surface.
...
- Runtime WebSocket handshake, auth denial, ordered delivery, replay cursor, and heartbeat framing.
