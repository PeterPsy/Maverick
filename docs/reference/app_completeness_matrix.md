...
| `design-studio` | yes | app-local contract tests plus sidecar proxy coverage | Source-available sandbox app with frontend/backend, CLI/MCP, lifecycle hooks, `design_project` references, standard view state, Storage import/export, bundled `design-studio-ops` skill, and a governed OpenDesign-compatible HTTP sidecar stub. Full OpenDesign vendoring, streaming/SSE proxy, provider proxy handlers, terminal, Local CLI, and host-folder import remain intentionally outside this sandbox MVP. |
