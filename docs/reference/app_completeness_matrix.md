...
| `design-studio` | yes | app-local contract tests plus ASGI sidecar proxy coverage | Source-available sandbox app with frontend/backend, CLI/MCP, lifecycle hooks, `design_project` references, standard view state, Storage import/export, bundled `design-studio-ops` skill, a governed OpenDesign sidecar launcher for the pinned curated daemon bundle, streaming/SSE proxy support, and core-handled sidecar routes for sanitized provider config plus Storage import/export. The diagnostics fallback is disabled in the declared runtime; real external provider adapters, terminal, Local CLI, and host-folder import remain intentionally outside this sandbox MVP. |
