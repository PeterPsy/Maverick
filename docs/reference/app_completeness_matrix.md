...
| `senses` | yes | app-local contract tests plus repo-wide baseline | Sealed sandbox app with frontend/backend, CLI/MCP discovery, install/migrate/health hooks, user-session pairing and device registry, Storage-backed frame ingestion, capture data events, and required Storage dependency aliases; runtime routing, device-token ingress, and capture references remain deferred. |
