...
| `crm` | yes | repo-wide baseline plus app-local coverage when present | Reference app for record-centric view-state and reference metadata. |
...
| `gmail-app` | yes | repo-wide baseline | Forkable app with Gmail workflow skill, email references, standard inbox view state, and app-owned SQLite state. |
| `maverick-monitor` | yes | repo-wide baseline | Frontend, backend, CLI, and dashboard view state are declared; MCP and references remain intentionally absent until implemented. |
