...
Suite 63 / TCB manifest 53 / hosted adapter 56 / recipe 26 retain the P5
...
| Live capability probe | operator-only Core-managed stateless history, two sequential real-filesystem-list calls, and one explicitly tool-less final response at the certificate-bound `high` effort (three requests total), with exact OpenAPI/model preflight before every generation | manifest step available; not run for r59 |
...
Revision 61 pins hosted adapter 50 and governed recipe 25 while advancing to
...
redaction is unchanged. OpenRouter profile 60 advances with the shared
adapter/TCB identity. Codex revision 15 remains byte-identical.

Revision 62 pins hosted adapter 51 and governed recipe 25 while advancing to
suite 58, matrix `2026-09-10-r58-p6-workspace-runtime-root-tcb48`, and TCB
manifest v48. The hosted tool factory now gives confined shell and managed
process capabilities the workspace-level private `runtime/` root required by
their contract, instead of the individual provider session root. Provider
conversation state remains session-scoped. OpenRouter profile 61 advances with
the shared adapter/TCB identity. Codex revision 15 remains byte-identical.

Revision 63 pins hosted adapter 52 and governed recipe 25 while advancing to
suite 59, matrix
...
non-writable Bubblewrap artifact by digest before launch. Google hosted wire
behavior and exact catalog target are unchanged. OpenRouter profile 62 advances
with the shared adapter/TCB identity. Codex revision 15 remains byte-identical;
its active authority must still pass the post-restart contract check.
...
Revision 67 pins hosted adapter 56 and governed recipe 26 while advancing to
suite 63, matrix `2026-09-12-r63-openrouter-glm-full-workspace-relace-tcb53`, and TCB
manifest v53. The provider-step output allowance is now 4,096 tokens, while
...
The OpenRouter GLM profile 2 advances independently with recipe 28 and the
