...
The current OpenRouter target uses hosted adapter 59, GLM recipe 29, suite 67,
TCB manifest 57, and profile 6. Google profile 70 and Antigravity remain
...
OpenRouter profile 6 allows `public`, `workspace_internal`, `personal_data`,
...
version-fenced workspace instructions and tool results are redacted; the same
paths in user input and other provenances are denied. Recognizable
...
`core-capability`. Google remains contained; OpenRouter profile 6 uses the
