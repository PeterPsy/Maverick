...
Suite 20 retains the P2 crash/pairing/outbox matrix and P3 finalization
...
It additionally binds recipe/catalog identity, compiler revision 4,
`codex-baseline-v4`, mandatory commit-bound instruction digests and governed
shell/process effect overlays, semantic compaction schema 3, artifact-backed
...
- a synthetic-only provider credential delivered only to the operator-controlled
  live-probe worker;
- the dated suite-v20 matrix revision
  `2026-08-29-r20-p4-adversarial-closure-tcb10` declared by the provider certificate module;
...
version `20`, matrix revision
`2026-08-29-r20-p4-adversarial-closure-tcb10`, and the OpenRouter
manifest. The Google suite uses version `20` and the same matrix revision. The
