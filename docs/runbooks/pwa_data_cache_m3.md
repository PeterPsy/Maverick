...
Every resource needs a non-empty user, workspace, app, resource, entity, and
policy revision. The entry key also includes entry schema version. The app
declares canonical data class and provenance, but the SDK derives the only
allowed local results: `deny`, `session`, or `cache`.

Persistent writes require all applicable conditions:

1. exact policy revision v2;
2. resource not in the invariant agentic control-plane denylist;
...
2. construct the client only from freshly authenticated principal context;
...
A minimal public adapter has this shape:

```ts
const client = createPwaCacheClient({
  appId: "example",
...
- expired or invalid entry: delete it and treat it as a miss;
...
markers are written before security-sensitive deletion and resumed on the next
successful bootstrap if the browser interrupts the operation.
...
static service-worker caches, OPFS, or unrelated origin storage.
...
2. Abort the v2 upgrade at the payload-split step.
3. Confirm IndexedDB remains at v1 and the original record is intact.
4. Reopen without fault injection and confirm the atomic v2 split succeeds.

### Interrupted cleanup

1. Write a durable cleanup marker and interrupt deletion.
2. Reinitialize the database.
3. Confirm matching entries are removed before reads and the marker disappears.
