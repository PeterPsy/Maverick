...
export class IncrementalSha256 {
  ...
  update(input: Uint8Array): this {
    ...
    while (offset < input.byteLength) {
      const take = Math.min(input.byteLength - offset, 64 - this.blockLength);
  ...
  digest(): Uint8Array {
    ...
    this.state.forEach((word, index) => writeUint32(output, index * 4, word));
    return output;
  }
  ...
  private compress(chunk: Uint8Array): void {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const first = (h + upperSigma1 + choose + ROUND_CONSTANTS[index] + this.words[index]) >>> 0;
