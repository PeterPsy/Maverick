"""Factory-use and runtime-registry checks for mutation retries."""
...
FACTORY_CALL = re.compile(r"\bcreateMutationRetryContract\s*\(")
RAW_DEDUPLICATION_CONTRACT = re.compile(r"\bserverDeduplicates\b")
FACTORY_IMPLEMENTATION = Path("packages/pwa-cache/src/retryPolicy.ts")
...
def production_retry_audit_ids(
    ...
    """Discover only direct typed-factory uses across every production source tree.

    RetryCoordinator rejects every structurally forged object at runtime. This scan
    therefore bans the old raw ``serverDeduplicates`` shape and requires each
    production factory call to expose a literal audit identity and target for CI
    evidence/parity checks. Computed values cannot silently escape discovery.
    """
    ...
    for path in production_source_paths(root):
        ...
        if RAW_DEDUPLICATION_CONTRACT.search(source):
            errors.append(
                ...
                "use the createMutationRetryContract() typed factory"
            )
        ...
        for open_parenthesis in starts:
            ...
            if body is None:
                errors.append(f"{relative}: malformed createMutationRetryContract() call")
                continue
            ...
            if audit_id is None:
                errors.append(
                    f"{relative}: createMutationRetryContract() requires one literal auditId"
                )
                continue
...
def literal_property(call_body: str, property_name: str) -> str | None:
    ...
    matches = [match.group("value") for match in pattern.finditer(call_body)]
