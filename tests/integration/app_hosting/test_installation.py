...
class TestAppInstallation(AppHostingTestBase):
    """Focused test slice."""
    ...
    def test_store_app_install_can_use_local_alias_without_losing_public_identity(self) -> None:
        store = self.make_store()
        now = datetime.now(tz=UTC)
        with TemporaryDirectory() as temp_dir:
            repo_root = self.make_repo_root(temp_dir)
            app_root = repo_root / "apps" / "vendor-records"
            self.write_contract(app_root, app_id="vendor-records", name="Vendor Records")
            source = register_app_source_from_contract(
                store,
                source_kind="platform",
                source_path=str(app_root),
            ...
            binding = install_store_app(
                ...
                source_id=source.source_id,
                workspace_id="default",
                local_app_id="sample-app",
                start_path=repo_root,
                now=now,
            )

            self.assertEqual(source.app_id, "vendor-records")
            self.assertEqual(source.public_app_id, "vendor-records")
            self.assertEqual(binding.public_app_id, "vendor-records")
            self.assertEqual(binding.local_app_id, "sample-app")
            self.assertEqual(binding.mount_app_id, "sample-app")
            self.assertEqual(binding.app_id, "sample-app")
            self.assertTrue((repo_root / "workspaces" / "default" / "data" / "sample-app").is_dir())
            self.assertFalse((repo_root / "workspaces" / "default" / "data" / "vendor-records").exists())
            self.assertEqual(store.get_workspace_app_binding(workspace_id="default", app_id="sample-app").source_record_id, source.source_id)
