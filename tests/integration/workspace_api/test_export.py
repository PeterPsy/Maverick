"""Tests for workspace export behavior."""
...
class WorkspaceExportTestCase(unittest.TestCase):
    ...
    def test_export_bundle_excludes_runtime_logs_tmp_and_keeps_data_and_storage(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            exported_paths = {
                ...
                workspace_root / "data" / "sample-app" / "record.json": "{}",
                workspace_root / "data" / "sample-app" / "cache" / "index.json": "{}",
            }
            excluded_paths = {
    ...
    def test_export_bundle_excludes_secrets_and_external_symlinks(self) -> None:
        with TemporaryDirectory() as temp_dir:
            workspace_root = Path(temp_dir) / "workspaces" / "default"
            safe = workspace_root / "storage" / "generated" / "report.md"
            env_file = workspace_root / "data" / "sample-app" / ".env"
            key_file = workspace_root / "data" / "sample-app" / "private.pem"
            external_target = Path(temp_dir) / "outside-secret.txt"
            external_link = workspace_root / "storage" / "generated" / "outside-link.txt"
            ...
            self.assertEqual([path.relative_to(workspace_root).as_posix() for path in discovered], ["storage/generated/report.md"])
