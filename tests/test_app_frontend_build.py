...
from core.apps.contracts import build_app_contract, build_app_entrypoints, build_app_lifecycle
...
from tests.phase4_app_hosting_helpers import Phase4AppHostingBase


class AppFrontendBuildTests(Phase4AppHostingBase):
    ...
    def write_buildable_app(self, repo_root, app_id: str = "buildable"):
        ...
        self.write_contract(
            ...
            contract=build_app_contract(
                lifecycle=build_app_lifecycle(rebuild=True),
    ...
    def test_frontend_build_command_runs_declared_app_build_and_publishes_event(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(run.call_args.kwargs["cwd"], app_root / "frontend")
    ...
    def test_frontend_build_command_allows_sandbox_runtime_agent(self) -> None:
        with TemporaryDirectory() as temp_dir:
            repo_root = self.make_repo_root(temp_dir)
            app_root = self.write_buildable_app(repo_root)
            store = self.make_store()
    ...
    def test_frontend_build_command_requires_rebuild_lifecycle(self) -> None:
        with TemporaryDirectory() as temp_dir:
            repo_root = self.make_repo_root(temp_dir)
            app_root = self.write_buildable_app(repo_root)
            self.write_contract(
                ...
                app_id="buildable",
                contract=build_app_contract(
                    lifecycle=build_app_lifecycle(rebuild=False),
                    entrypoints=build_app_entrypoints(frontend="frontend/dist"),
                ),
            )
            store = self.make_store()
