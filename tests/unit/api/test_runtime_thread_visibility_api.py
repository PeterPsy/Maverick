...
class RuntimeThreadVisibilityApiTestCase(AppReferenceApiTestSupport, unittest.TestCase):
    ...
    def test_runtime_thread_rest_catalog_uses_summary_payload_without_duplicate_items(self) -> None:
        ...
        listed_threads = list_payload["threads"]
        self.assertEqual(len(listed_threads), 50)
        ...
        self.assertEqual((list_payload["threads_page"]["total"], list_payload["threads_page"]["filtered_total"]), (55, 55))
        self.assertTrue(all("system_prompt" not in thread and "provider_id" not in thread for thread in listed_threads))
        self.assertTrue(all("title_generation_input_hash" not in thread for thread in listed_threads))
        self.assertTrue(all((thread["runtime_profile"], thread["device_use_enabled"]) == ("workspace", False) for thread in listed_threads))
