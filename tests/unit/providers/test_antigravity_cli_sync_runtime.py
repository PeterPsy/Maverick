"""Antigravity ownership across synchronous Core service boundaries."""
...
from core.runtime.agentic_runtime_service import (
    cancel_agentic_runtime,
    close_agentic_runtime,
...
class FixtureRuntimeStore:
    """Only persistence is in-memory; lifecycle and execution remain real."""
...
class AntigravityCliSyncRuntimeTest(AntigravityCliFixture, unittest.TestCase):
    def setUp(self):
    ...
    def execute(self, prompt, **kwargs):
        return execute_runtime_turn(
            ...
            provider=build_antigravity_cli_candidate_definition(),
            input_text=prompt,
            agentic_adapter=self.controller,
            provider_state=self.store.state,
            correlation_id="turn",
            effective_authority=self.authority,
            launch_spec=self.spec,
            timeout_seconds=3,
            **kwargs,
        )

    def test_two_consecutive_sync_turns_keep_connection_live(self):
    ...
    def test_sync_prepare_two_turns_and_idempotent_cleanup(self):
        ...
        self.assertEqual(self.store.state.provider_thread_id, "fixture-conversation")
    ...
    def test_sync_interrupt_active_turn_then_resume_and_execute(self):
        prepared = prepare_agentic_runtime(
            self.store,
            session_id="test",
            adapter=self.controller,
        ...
        with ThreadPoolExecutor(max_workers=1) as worker:
            ...
            with self.assertRaises((NativeStructuredCliError, asyncio.CancelledError)):
        ...
        self.assertEqual(
            resumed.provider_state_updates["provider_thread_id"],
            "fixture-conversation",
        )
        self.assertEqual(
            self.execute("after-interrupt").output_text,
            "answer:after-interrupt",
        )

    def test_sync_interrupt_during_preparation_reaps_process_and_loop(self):
        spec = replace(
            self.spec,
            env_overrides={
    ...
    def test_sync_steering_reports_safe_next_turn_and_interrupts_cleanly(self):
        accepted = Event()
        with ThreadPoolExecutor(max_workers=1) as worker:
            turn = worker.submit(
                self.execute,
            ...
            result = run_runtime_coroutine(
                self.controller.steer(NativeSteerContext("test", "changed"))
            )
            self.assertEqual(result.status, "not_supported")
            self.assertEqual(result.reason, "antigravity_safe_next_turn_only")
            cancel_agentic_runtime(
                self.store,
                session_id="test",
                correlation_id="turn",
    ...
    def test_sync_cleanup_unblocks_active_turn(self):
        accepted = Event()
        with ThreadPoolExecutor(max_workers=1) as worker:
            turn = worker.submit(
                self.execute,
    ...
    def test_sync_callback_failure_leaves_no_worker(self):
        prepared = prepare_agentic_runtime(
            self.store,
            session_id="test",
            adapter=self.controller,
