...
class ContinuationHandoffLegacyDigestTestCase(unittest.TestCase):
    def test_current_handoff_round_trip_preserves_snapshot(self) -> None:
        handoff = _continuation_handoff()

        self.assertEqual(runtime_continuation_handoff_from_document(asdict(handoff)), handoff)

    def test_handoff_updates_validated_legacy_target_digest(self) -> None:
        handoff = _continuation_handoff()
        serialized = _legacy_handoff_document(handoff)

        self.assertEqual(runtime_continuation_handoff_from_document(serialized), handoff)
        self.assertNotEqual(serialized["target_binding_digest"], handoff.target_binding_digest)

    def test_handoff_rejects_target_digest_not_bound_to_original_document(self) -> None:
        handoff = _continuation_handoff()
        serialized = _legacy_handoff_document(handoff)
        serialized["target_binding_digest"] = handoff.target_binding_digest
    ...
    def test_handoff_rejects_tampered_embedded_binding(self) -> None:
        serialized = _legacy_handoff_document(_continuation_handoff())
        serialized["target_execution_binding"]["adapter_artifact_digest"] = "c" * 64
...
def _legacy_handoff_document(handoff: RuntimeContinuationHandoff) -> dict:
    serialized = asdict(handoff)
    binding = serialized["target_execution_binding"]
    binding["adapter_artifact_digest"] = binding.pop("adapter_identity_digest")
    binding["binding_digest"] = canonical_digest(binding)
    serialized["target_binding_digest"] = binding["binding_digest"]
    return serialized
