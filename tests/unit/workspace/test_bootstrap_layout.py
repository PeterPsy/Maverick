...
class WorkspaceBootstrapTestCase(unittest.TestCase):
    ...
    def make_temp_repo_root(self) -> Path:
        """Create a minimal Maverick-shaped repository root for isolated tests."""
    ...
    def test_workspace_id_boundary_matches_error_message(self) -> None:
        with self.assertRaisesRegex(InvalidWorkspaceIdError, "2-64 chars"):
            ensure_workspace_layout(workspace_id="a", start_path=Path(__file__))
        self.assertEqual(ensure_workspace_layout(workspace_id="ab", start_path=self.make_temp_repo_root()).workspace_id, "ab")
    ...
    def test_app_and_runtime_paths_align_with_workspace_contract(self) -> None:
        ...
        self.assertEqual(workspace_app_data_root("acme", "sample-app", start_path=repo_root), workspace.data / "sample-app")
    ...
    def test_file_identity_survives_rename_inside_workspace(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            data_root = workspace_root / "data" / "sample-app"
            ...
            self.assertEqual(renamed_identity.relative_path, "data/sample-app/state-renamed.json")
    ...
    def test_platform_bootstrap_removes_legacy_workspace_provider_home(self) -> None:
        repo_root = self.make_temp_repo_root()
        legacy_home = repo_root / "workspaces" / "default" / "runtime" / "codex-home"
        legacy_home.mkdir(parents=True)
        (legacy_home / "config.json").write_text("{}", encoding="utf-8")

        with patch.dict("os.environ", {"MAVERICK_ALLOW_INSECURE_TEST_DEFAULTS": "1"}, clear=False):
            bootstrap_platform_state(start_path=repo_root)

        self.assertFalse(legacy_home.exists())

    def test_platform_bootstrap_does_not_run_recovery_unless_requested(self) -> None:
        repo_root = self.make_temp_repo_root()

        with patch.dict("os.environ", {"MAVERICK_ALLOW_INSECURE_TEST_DEFAULTS": "1"}, clear=False), patch(
            "core.api.platform_state.recover_interrupted_runtime_turns_after_backend_restart"
        ) as recover:
            bootstrap_platform_state(start_path=repo_root)
